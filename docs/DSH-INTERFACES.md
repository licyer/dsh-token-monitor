...
> 当前基线：实测环境 DSH **0.1.2-rc.1**；≤0.1.1 分支为按旧契约写的回退路径（当前环境无法回归）。
...
| 客户端平台/注入声明 | `dsh.client.platform: "web"`、`dsh.client.inject`（package.json） | 声明注入的宿主插件 | `inject` 在 0.1.2 为 **informational**（官方注释），包不存在也不报错；对 0.1.1 仍有意义 → 保留 | 0.1.1 宿主插件清单变化时更新 |
...
| 触点 | 定位检索词 | 用途 | 兼容面 / 备注 | 升级核对点 |
|---|---|---|---|---|
| 头部右侧槽 | `slots.inject("conversation.session.header.utilities"` | 徽标入口（id `token-monitor`, order -10） | 两版槽名相同 | 槽 key / order 语义 |
...
| 触点 | 定位检索词 | 用途 | 兼容面 / 备注 | 升级核对点 |
|---|---|---|---|---|
| Web 路由注册 | `ctx.webServer.register({ kind: "exact", path, handler })`（lib/index.js 多处 + `ROUTES` 数组） | 全部 `/token-monitor/*` 接口（overview / usage / config / import-export / version / upgrade / echarts 静态） | 0.1.2 实测正常 | register 契约、路由认证基座（web token） |
...
| 触点 | 定位检索词 | 用途 | 兼容面 / 备注 | 升级核对点 |
|---|---|---|---|---|
| 插件自升级 | `spawn("dsh", ["plugin", "--profile", "web", "update", "dsh-token-monitor"])`（index.js） | 设置页“升级”按钮 | 通用 | CLI 子命令/参数 |
