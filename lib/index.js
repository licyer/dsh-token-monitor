...
const VERIFIED_PROVIDERS = new Set(['kimi-coding', 'moonshotai-cn', 'deepseek', 'opencode-go']);
...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => {
    ...
    const adaptersDispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          const map = store.getProviderMappings();
          ...
          for (const item of adapters) {
            ...
            if (groupIdx[v] === undefined) {
              ...
              groups.push({ vendor: v, providers: [] });
            }
    ...
    return () => { dispose(); currentDispose(); adaptersDispose(); };
  ...
  // 提供方映射种子：启动时幂等写入 provider_mappings。
  // seed 的 provider_name 是权威显示名（我们维护）：不存在则插入，存在但名称不同则同步更新
  // （vendor 保留用户值）；这样映射表改名后旧库也会随之更新，弹层/全部提供方与用量页一致。
  {
    const existing = store.getProviderMappings();
    for (const m of PROVIDER_MAPPING_SEED) {
      const cur = existing[m.providerId];
      if (!cur) {
        store.upsertProviderMapping(m);
      } else if (cur.provider_name !== m.providerName) {
        // 名称以 seed 为准同步；vendor/sort_order 保留库里的值（无则回退 seed）
        store.upsertProviderMapping({
          providerId: m.providerId,
          providerName: m.providerName,
          vendor: cur.vendor || m.vendor,
          sortOrder: cur.sort_order ?? m.sortOrder,
        });
      }
    }
  }
  ...
  const ROUTES = [
    ...
    },
    {
      ...
      handle(days, url) {
        ...
        const now = Date.now();
        const isYesterday = !day && days === -1;
        const where = day ? 'r.day = ?' : isYesterday ? 'r.day >= ? AND r.day < ?' : 'r.day >= ?';
        const arg = day || (isYesterday ? [dayOf(now - 86_400_000), dayOf(now)] : windowStartDay(days));
