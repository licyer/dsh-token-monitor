    ...
    const disposers = [
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            // 只有 npm 渠道做版本比较；github（main 分支可能领先 npm，版本号不可比）与
            // link（本地开发）不提示更新
            const latest = inst.channel === 'npm' ? await getLatestVersion() : null;
            sendJson(res, 200, {
              ok: true,
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            // 非 npm 渠道不升级：link 会把开发挂载覆盖成发布版；github 渠道暂无 SHA 对比，暂不支持
            const inst = installChannel();
            if (inst.channel !== 'npm') {
              sendJson(res, 400, { ok: false, error: inst.channel === 'link' ? 'linked-dev' : 'non-npm-channel' });
              return;
            }
            const result = await new Promise((resolve) => {
              // update（非 add）：语义正确的升级命令；npm 渠道升到 registry 最新版
              // （github 渠道 pnpm update 会重拉 main，但渠道守卫已拒绝，未来做 SHA 检测后可放行）
