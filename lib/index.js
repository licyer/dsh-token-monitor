...
import { loadPricing, seedModelPrices, p4ToYuan, yuanToP4 } from './util/pricing.js';
...
const VERIFIED_PROVIDERS = new Set(['kimi-coding', 'moonshotai-cn', 'deepseek', 'opencode-go', 'openrouter']);
...
export function apply(ctx, config = {}) {
  ...
  let versionCache = { at: 0, latest: null }; // 最新版缓存 5 分钟

  async function getLatestVersion() {
    if (versionCache.latest && Date.now() - versionCache.at < 300_000) return versionCache.latest;
    try {
      const res = await fetchJson(NPM_LATEST_URL);
      const v = res && res.ok && res.data && res.data.version;
      if (typeof v === 'string' && v) versionCache = { at: Date.now(), latest: v };
    } catch { /* 网络失败：保持缓存 */ }
  ...
  function isNewer(latest, current) {
    ...
    const a = p(latest); const b = p(current);
    for (let i = 0; i < 3; i++) { if (a[i] !== b[i]) return a[i] > b[i]; }
    return false;
  }

  /** 插件安装渠道：从 profile package.json 的依赖声明判定（npm 版本号 / github: / link:），
   *  同时返回依赖原文（设置页副标题展示用）。 */
  function installChannel() {
    try {
      const profPkg = JSON.parse(fs.readFileSync(path.join(dshHome, 'profiles', 'web', 'package.json'), 'utf8'));
      const dep = profPkg && profPkg.dependencies && profPkg.dependencies['dsh-token-monitor'];
      if (typeof dep !== 'string') return { channel: 'unknown', dep: null };
      if (dep.startsWith('link:')) return { channel: 'link', dep };
      if (dep.startsWith('github:')) return { channel: 'github', dep };
      return { channel: 'npm', dep };
    } catch {
      return { channel: 'unknown', dep: null };
    }
  }
    ...
    const disposers = [
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            const inst = installChannel();
            // npm / github 渠道都做 npm latest 比较（发布纪律：先推 GitHub 再发 npm，
            // 两边版本号一致，npm 最新版即可代表 GitHub main 的最新发布点）；link 不提示
            const latest = inst.channel === 'npm' || inst.channel === 'github' ? await getLatestVersion() : null;
            sendJson(res, 200, {
              ok: true,
              current: PLUGIN_VERSION,
              latest: latest || null,
              hasUpdate: latest ? isNewer(latest, PLUGIN_VERSION) : false,
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            // link 渠道不升级（会把开发挂载覆盖成发布版）；npm/github 都放行
            // （pnpm update 对 npm 依赖升 registry 最新版，对 github: 依赖重拉 main 分支）
            const inst = installChannel();
            if (inst.channel === 'link') {
              sendJson(res, 400, { ok: false, error: 'linked-dev' });
              return;
            }
            const result = await new Promise((resolve) => {
              // update（非 add）：语义正确的升级命令；npm 升 registry 最新版，github 重拉 main
              const child = spawn('dsh', ['plugin', '--profile', 'web', 'update', 'dsh-token-monitor'], { shell: true });
              let out = '';
              const timer = setTimeout(() => { try { child.kill(); } catch { /* 忽略 */ } resolve({ ok: false, error: '升级超时（60s）' }); }, 60_000);
              child.stdout.on('data', (c) => { out += c; });
              child.stderr.on('data', (c) => { out += c; });
              child.on('close', (code) => { clearTimeout(timer); resolve(code === 0 ? { ok: true } : { ok: false, error: out.slice(-500) || `exit ${code}` }); });
              child.on('error', (e) => { clearTimeout(timer); resolve({ ok: false, error: errorText(e) }); });
            });
