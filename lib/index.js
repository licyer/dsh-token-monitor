  ...
  function foldOnce() {
    ...
    const task = Promise.resolve().then(() => {
      try {
        ...
        // 回归哨兵：会话目录存在却一个日志文件都没匹配到 = DSH 的日志命名约定变了。
        // 这条路径过去既不报错也不打日志（文件名写死 + existsSync，0.1.5 改名后采集
        // 静默停摆），必须留下痕迹，否则同类问题只能靠肉眼发现。
        if (r.sessionDirs > 0 && r.filesScanned === 0) {
          ctx.logger.warn(
            `token-monitor: 发现 ${r.sessionDirs} 个会话目录但未匹配到任何日志文件——`
            + 'DSH 的会话日志命名约定可能已变化，请核对 $DSH_HOME/sessions 下的实际文件名',
          );
        }
  ...
  ctx.effect(() => {
    ...
    const dispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        refreshUsdCnyRate();
        // 每来源最近有记录的时间（数据来源卡"上次更新"列；无记录为 null）
        const lastUpdated = {};
