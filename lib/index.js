...
import { loadPricing, seedModelPrices, syncBuiltinPrices, backfillCosts, p4ToYuan, yuanToP4 } from './util/pricing.js';
...
/** 失败结果的最短缓存：网络瞬时故障只影响几秒，而不是整个 cacheMs（默认 60s）。 */
...
export function apply(ctx, config = {}) {
  ...
  async function providerOverview(id, force) {
    const cached = cache.get(id);
    // 成功（以及"未配置 key"这类稳定态）照旧按 cacheMs 缓存；失败——抓取器返回的 ok:false，
    // 或抓取器抛错时的兜底 payload——只按 FAILED_CACHE_MS 缓存：一次网络抖动不再把「查询失败」
    // 钉在界面上整整一分钟，下一轮轮询就能自愈。
    // force=true（手动刷新）跳过缓存直接抓：否则点刷新按钮拿到的还是缓存里的旧数据
    const ttl = cached && cached.payload && cached.payload.ok === false && cached.payload.configured !== false
      ? Math.min(cacheMs, FAILED_CACHE_MS)
      : cacheMs;
    if (!force && cached && Date.now() - cached.at < ttl) return cached.payload;
  ...
  async function overview(force) {
    ...
    const settled = await Promise.all(routes.map(async (route) => {
      ...
      if (FETCHERS[pid]) {
        try {
          return await providerOverview(pid, force);
        } catch (error) {
    ...
    const dispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const body = JSON.stringify(await overview(force));
          res.writeHead(200, {
    ...
    const currentDispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (pid && FETCHERS[pid]) {
            try {
              provider = await providerOverview(pid, force);
            } catch (error) {
  ...
  // 启动即把【新增/变更】的内置价同步到已有库：seedModelPrices 只在空表时写入，
  // 后续版本新加的模型（如 deepseek-flash）不会自动入库——必须每次启动核对补齐。
  // syncBuiltinPrices 幂等且尊重用户自定义（有 custom 行的 model 整个跳过），
  // 完全一致时不删不插（真正"只入库一次"）。
  syncBuiltinPrices(store.db, ctx.logger);
  ...
  // 历史费用回填：此前"定价未就绪/缺失"导致 cost_usd_nano 为 NULL 的记录，
  // 在定价可用后按原事件时间重算（含 rollup 差额与 unpriced 计数修正）。幂等。
  backfillCosts(store.db, pricing, ctx.logger);
  // 释放：Cordis 没有 'dispose' 事件（只 emit internal/dispatch|plugin|status），
  // 原先的 ctx.on('dispose', …) 是死代码；清理必须走 ctx.effect 的返回值。
  ...
  const ROUTES = [
    ...
    },
    {
      ...
      handle() {
        ...
        // 模型归属哪个供应商：取**用量最大**的那条路由，而不是"逐行覆盖、最后写入者获胜"。
        // 起因：同一模型可能被多条路由写过（如 deepseek-v4-flash 既走 deepseek 官方，
        // 也走过 opencode-go），逐行覆盖时归属取决于 GROUP BY 的返回顺序 —— 既会把
        // 用量最大的官方路由判给代理（图例里"掉进 opencode 组"），又会在切换时间窗
        // 后归属跳变。改为按 vendor 汇总 token 取最大者，归属稳定且符合直观。
        const modelProv = {};
        const modelVendorTok = {}; // model -> { vendor -> tokens }
        for (const r of rows) {
          const m = r.model || 'unknown';
          const v = vendorOf(r.provider || 'unknown');
          const byVendor = modelVendorTok[m] || (modelVendorTok[m] = {});
          byVendor[v] = (byVendor[v] || 0) + (r.tokens || 0);
        }
        for (const m of Object.keys(modelVendorTok)) {
          let bestVendor = null;
          let bestTok = -1;
          for (const v of Object.keys(modelVendorTok[m])) {
            if (modelVendorTok[m][v] > bestTok) { bestVendor = v; bestTok = modelVendorTok[m][v]; }
          }
          modelProv[m] = bestVendor;
        }
  ...
  ctx.effect(() => {
    ...
    const dispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        refreshUsdCnyRate();
        // 数据来源卡两列取自**同一条** sync_logs 行，保证"时间 ↔ 条数"严格配对：
        //   「最近更新」= 该行的 finished_at（这次同步动作何时完成）
        //   「最近同步」= 该行的 imported（这次同步进了多少条）
        // 口径限定：只看**明细**导入——fold / db-scan / file-import / sql-import，
        // 排除 db-rollup（那是 cc-switch 的"按天聚合"表，75 行是 75 天、不是 75 条请求，
        // 量纲不同）与 prune（清理审计，本就不是同步）；
        // 且优先取最近一条 imported > 0 的行（该源从未有过新增时退回最近一条）——
        // 空转轮（每 5 分钟一轮、无新数据）既不会把上一轮的条数冲成 0，也不会让时间虚新。
        // 为什么不再用"库里最新一条记录的事件时刻"：那是数据**内容**的时间，与"我这次同步了
        // 多少条"不是同一件事，两列并排会互相矛盾（导入完时间不动、或条数与时间对不上）。
        const lastSync = {};
        ...
        // 不再单独下发 lastUpdated：它与 lastSync[s].at 是同一个值（重复字段），
        // 前端「最近更新 / 最近同步」两列都从 lastSync 读，天然配对。
        sendJson(res, 200, {
          ok: true,
  ...
  // 模型定价维护（model_prices 表，见 docs/模型定价表设计.md）：
