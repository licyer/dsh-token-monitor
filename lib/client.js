...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function UsageView() {
			...
			// 导入 CC 历史：POST 导入接口，成功后静默重载数据
			function importCc() {
				fetch("/token-monitor/import/cc-switch", { method: "POST" })
					.then(function (r) { return r.ok ? r.json() : null; })
					.then(function (d) {
						if (d && !d.error) load(days, scope, client, provider, model, focus, true);
					})
					.catch(function () {});
			}
			...
			function confirmDeleteCc() {
				...
				fetch("/token-monitor/import/cc-switch", { method: "DELETE" })
					.then(function (r) { return r.ok ? r.json() : null; })
					.then(function (d) {
						if (d && d.ok) load(days, scope, client, provider, model, focus, true);
					})
					.catch(function () {});
			}
