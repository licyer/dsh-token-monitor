	...
	factory: (require) => {
		...
		function TokenMonitorEntry(props) {
			...
			if (fetchError && !overview) {
				...
				label = (current ? current.model : currentProvider.label) + " · " + quotaValue;
		...
		function apply(ctx) {
			...
			// - 桌面：完整文案（含模型名）带 max-width + 省略号，兜住异常长文案（如"未配置 API Key"）
			//   不撑破头部工具条——官方该行是 flex:none，永远不会自己收缩；
			...
			if (typeof document !== "undefined" && !document.querySelector("style[data-plugin-css=\"tm-entry-label\"]")) {
				...
				entryLabelTag.textContent =
					...
					".tm-entry-label-full{max-width:220px}" +
					".tm-entry-label-short{display:none;max-width:110px}" +
