...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 趋势图序列：左轴 token（新增输入/输出，独立渐变面积），右轴按 metric 切换（费用/请求次数）。 */
		...
		/** 趋势图 option：token 序列走左轴，右轴按 metric（费用/请求次数），缓存命中带面积填充。 */
		function buildTrendOption(data, metric) {
			...
			return {
				...
				grid: { left: 12, right: 12, top: 16, bottom: 54, containLabel: true },
				legend: {
					...
					formatter: function (params) {
						...
						for (var i = 0; i < params.length; i++) {
							...
							lines.push(p.marker + " " + p.seriesName + "："
								+ (p.seriesName === "预估费用" ? fmtUsd(p.value) : fmtTokens(p.value)));
						}
				...
				series: trendSeries(metric).map(function (s) {
					var item = {
						...
						lineStyle: { width: s.stack ? 1 : 2, color: s.color, type: s.dashed ? "dashed" : "solid" },
			...
			return h("div", null,
				...
				h("div", { style: { display: "flex", alignItems: "center", gap: "2px" } },
					...
					h("span", { style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" } },
						"（实时增量 · 受筛选条件影响 · 缓存命中与新增输入/输出量级差过大时，后者在图中几乎不可见，故不展示）"),
					h("span", { style: { flex: 1 } }),
		...
		function buildModelBarsOption(data, metric) {
			...
			for (var i = 0; i < data.models.length; i++) {
				(function (mi) {
					...
					series.push({
						name: m === "其他" ? m : modelLabel(m), type: "bar", stack: "tokens", yAxisIndex: 0, barMaxWidth: 36,
		...
		function ModelBarsView(props) {
			...
			// 聚合：供应商按窗口 token 总量降序；模型取 Top8 + 其他
			var data = React.useMemo(function () {
				var rows = props.rows || [];
				var tokOf = function (r) {
					return (r.input_tokens || 0) + (r.output_tokens || 0) + (r.cache_read_tokens || 0);
				};
				var provTok = {}, modelTok = {};
				for (var i = 0; i < rows.length; i++) {
					var r = rows[i];
					var p = providerLabel(r.provider);
					var m = r.model || "unknown";
					var t = tokOf(r);
					provTok[p] = (provTok[p] || 0) + t;
					modelTok[m] = (modelTok[m] || 0) + t;
				}
				// 供应商顺序统一规则：DeepSeek 永远第一（DSH 主场）→ 其余按首字母 →
				// 首字母相同按首次使用时间（providerOrder 路由的 first_use）
				var firstUse = {};
				var order = props.providerOrder || [];
				for (var oi = 0; oi < order.length; oi++) firstUse[providerLabel(order[oi].provider)] = order[oi].first_use;
				var providers = Object.keys(provTok).sort(function (a, b) {
					var pa = a === "deepseek" ? 0 : 1;
					var pb = b === "deepseek" ? 0 : 1;
					if (pa !== pb) return pa - pb;
					var alpha = a.localeCompare(b);
					if (alpha !== 0) return alpha;
					var fa = firstUse[a] !== undefined ? firstUse[a] : Infinity;
					var fb = firstUse[b] !== undefined ? firstUse[b] : Infinity;
					return fa - fb;
				});
				// 模型顺序：按供应商分组连续（同一供应商的模型在图例里排在一起），
				// 供应商内按 token 降序；跨供应商取 Top8 + 其他
				var modelProv = {};
				for (var i1 = 0; i1 < rows.length; i1++) {
					var r1 = rows[i1];
					var m1 = r1.model || "unknown";
					modelProv[m1] = providerLabel(r1.provider);
				}
				var models = [];
				for (var pi = 0; pi < providers.length && models.length < 8; pi++) {
					var inProv = Object.keys(modelTok).filter(function (m) { return modelProv[m] === providers[pi]; });
					inProv.sort(function (a, b) { return modelTok[b] - modelTok[a]; });
					for (var mi = 0; mi < inProv.length && models.length < 8; mi++) models.push(inProv[mi]);
				}
				var hasOther = Object.keys(modelTok).length > models.length;
				if (hasOther) models.push("其他");
				var tokens = {}, metricTotals = {};
				for (var i2 = 0; i2 < rows.length; i2++) {
					var r2 = rows[i2];
					var p2 = providerLabel(r2.provider);
					var m2 = r2.model || "unknown";
					var bucket = hasOther && models.indexOf(m2) < 0 ? "其他" : m2;
					tokens[p2] = tokens[p2] || {};
					tokens[p2][bucket] = (tokens[p2][bucket] || 0) + tokOf(r2);
					// 右柱：供应商合计（费用 或 请求次数）
					metricTotals[p2] = (metricTotals[p2] || 0)
						+ (metric === "cost" ? (r2.cost_usd || 0) : (r2.requests || 0));
				}
				return { providers: providers, models: models, tokens: tokens, metricTotals: metricTotals };
			}, [props.rows, metric]);

			var serialized = JSON.stringify({ d: data, m: metric });
			var option = React.useMemo(function () {
				return data.providers.length ? buildModelBarsOption(data, metric) : null;
			}, [serialized]);

			if (!data.providers.length) return h("div", { style: S.muted }, "窗口内暂无数据");
		...
		function CalendarHeatmap(props) {
			var agg = React.useMemo(function () {
				var byDay = {};
				var maxTok = 0;
				var rows = props.rows || [];
				for (var i = 0; i < rows.length; i++) {
					var r = rows[i];
					var t = (r.input_tokens || 0) + (r.output_tokens || 0) + (r.cache_read_tokens || 0) + (r.cache_write_tokens || 0);
					if (!byDay[r.day]) byDay[r.day] = { tokens: 0, requests: 0 };
					byDay[r.day].tokens += t;
					byDay[r.day].requests += r.requests || 0;
					if (byDay[r.day].tokens > maxTok) maxTok = byDay[r.day].tokens;
				}
				var p = function (x) { return String(x).padStart(2, "0"); };
				var now = new Date();
				var startDate = new Date(now.getFullYear(), now.getMonth() - 11, 1);
				// 起始月 1 号不是周一则向上补齐到该周周一（跨进上月按 0 渲染，标签隐藏）
				var padStartDays = (startDate.getDay() + 6) % 7;
				var displayStart = new Date(startDate.getTime() - padStartDays * 86400000);
				var leadingYM = padStartDays > 0
					? displayStart.getFullYear() + "-" + p(displayStart.getMonth() + 1)
					: "";
				// 日期序列：补齐后的周一 → 本月最后一天所在周的周日（首尾两列都铺满，多出按 0 渲染）
				var monthEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0);
				// firstDay=1（周一）时一周到周日结束：补 (7 - getDay()) % 7 天
				var padDays = (7 - monthEnd.getDay()) % 7;
				var endDate = new Date(monthEnd.getTime() + padDays * 86400000);
				var dayList = [];
				for (var dt = new Date(displayStart); dt <= endDate; dt = new Date(dt.getTime() + 86400000)) {
					dayList.push(dt.getFullYear() + "-" + p(dt.getMonth() + 1) + "-" + p(dt.getDate()));
				}
				var rangeStart = displayStart.getFullYear() + "-" + p(displayStart.getMonth() + 1) + "-" + p(displayStart.getDate());
				var rangeEnd = endDate.getFullYear() + "-" + p(endDate.getMonth() + 1) + "-" + p(endDate.getDate());
				// 补周末跨进了下个月时，该月的标签要隐藏（格子保留，标签不要）
				var overflowYM = padDays > 0 && endDate.getMonth() !== monthEnd.getMonth()
					? endDate.getFullYear() + "-" + p(endDate.getMonth() + 1)
					: "";
				return { byDay: byDay, dayList: dayList, maxTok: maxTok, rangeStart: rangeStart, rangeEnd: rangeEnd, overflowYM: overflowYM, leadingYM: leadingYM };
			}, [props.rows]);
		...
		function RequestRecords() {
			...
			var columns = [
				...
				{ key: "model", label: "模型名称", width: "10%", render: function (r) { return modelLabel(r.model); } },
		...
		/** 使用排行表：按 模型/供应商/客户端 三维度聚合；首维占首列，其余两维在尾部，
		 *  位置随模式互换（占比绑位置不绑字段）。 */
		function RankTable(props) {
			...
			var built = React.useMemo(function () {
				...
				for (var i = 0; i < rows.length; i++) {
					var r = rows[i];
					var key = dim === "provider" ? providerLabel(r.provider)
						: dim === "client" ? clientLabel(r.client)
						: (r.model || "unknown");
					var g = grouped[key];
					if (!g) g = grouped[key] = {
						name: key, requests: 0, input_tokens: 0, output_tokens: 0,
						cache_read_tokens: 0, cache_write_tokens: 0, cost_usd: 0,
						_ttftSum: 0, _ttftCount: 0, _models: [], _provs: [], _clients: [],
					};
					...
					if (r.ttft_avg_ms != null) { g._ttftSum += r.ttft_avg_ms * (r.requests || 0); g._ttftCount += r.requests || 0; }
					var ml = r.model || "unknown", pl = providerLabel(r.provider), cl = clientLabel(r.client);
					if (g._models.indexOf(ml) < 0) g._models.push(ml);
					if (g._provs.indexOf(pl) < 0) g._provs.push(pl);
					if (g._clients.indexOf(cl) < 0) g._clients.push(cl);
				}
				// 拼接顺序统一规则：DSH 永远在前，其余按首字母
				var rank2 = function (x) { return x === "DSH" ? "0" : "1" + x; };
				var join = function (arr) {
					return arr.slice().sort(function (a, b) { return rank2(a).localeCompare(rank2(b)); }).join(" | ");
				};
				var list = Object.keys(grouped).map(function (k) {
					var g = grouped[k];
					return {
						model: dim === "model" ? modelLabel(g.name) : join(g._models.map(modelLabel)),
						provider: dim === "provider" ? g.name : join(g._provs),
						client: dim === "client" ? g.name : join(g._clients),
						requests: g.requests,
						...
						cost_usd: g.cost_usd,
						ttft_avg_ms: g._ttftCount > 0 ? g._ttftSum / g._ttftCount : null,
					};
				});
				var tok = function (r) {
					return r.input_tokens + r.output_tokens + r.cache_read_tokens + r.cache_write_tokens;
				};
				list.sort(function (a, b) { return tok(b) - tok(a); });
				return list;
			}, [props.rows, dim]);
			...
			var DIM_LABEL = { model: "模型名称", provider: "供应商", client: "客户端" };
			...
			var nameCol = function (key, label, width) {
				return {
					...
					render: function (r) { return trunc20(r[key]); },
					title: function (r) { return String(r[key] ?? ""); },
				};
			};
			var cols = [nameCol(dim, DIM_LABEL[dim], "11%")].concat(METRIC_COLS);
			if (dim === "model") {
				cols.push(nameCol("provider", "供应商", "13.3%"), nameCol("client", "客户端", "13.3%"));
			} else if (dim === "provider") {
				cols.push(nameCol("model", "模型名称", "13.3%"), nameCol("client", "客户端", "13.3%"));
			} else {
				cols.push(nameCol("provider", "供应商", "13.3%"), nameCol("model", "模型名称", "13.3%"));
			}

			return built.length
				? h(UsageTable, { columns: cols, rows: built })
		...
		function UsageView() {
			...
			// 累计消耗图的数据：历史全量、不受筛选条件影响（独立拉取，只挂一次 + 点刷新时重拉）
			var _allTime = React.useState(null), allTime = _allTime[0], setAllTime = _allTime[1];
			var _provOrder = React.useState(null), provOrder = _provOrder[0], setProvOrder = _provOrder[1];
			var _calRows = React.useState(null), calRows = _calRows[0], setCalRows = _calRows[1];
			// "当天"窗口的小时级趋势数据（仅 days===1 时拉取）
			var _hourly = React.useState(null), hourly = _hourly[0], setHourly = _hourly[1];
			// 半小时级数据（<15 桶时加细刻度用，同样仅当天拉取）
			var _halfHourly = React.useState(null), halfHourly = _halfHourly[0], setHalfHourly = _halfHourly[1];
			...
			var load = React.useCallback(function (d, sc, cl, pv, md, fs, silent) {
				...
				// 累计消耗图：历史全量，只在非静默加载（首次挂载/手动刷新）时拉取，
				// 条件切换的静默加载不动它——免得"不受条件影响"的图跟着条件闪
				var includeAllTime = !silent;
				if (includeAllTime) {
					jobs.push(fetch(USAGE_URLS.byModel + "?days=3650", { headers: { accept: "application/json" } }).then(ok));
					jobs.push(fetch("/token-monitor/usage/provider-order", { headers: { accept: "application/json" } }).then(ok));
					jobs.push(fetch(USAGE_URLS.daily + "?days=365", { headers: { accept: "application/json" } }).then(ok));
				}
				// "当天"额外拉小时级 + 半小时级趋势（同样带筛选参数）
				if (d === 1) {
					...
					jobs.push(fetch(USAGE_URLS.hourly + hq + "&half=1", { headers: { accept: "application/json" } }).then(ok));
				}
				Promise.all(jobs).then(function (results) {
					...
					var idx = 3;
					if (includeAllTime) {
						setAllTime(results[idx++].data || []);
						setProvOrder(results[idx++].data || []);
						setCalRows(results[idx++].data || []);
					}
					setHourly(d === 1 && results[idx] ? (results[idx].data || []) : null);
					setHalfHourly(d === 1 && results[idx + 1] ? (results[idx + 1].data || []) : null);
				}).catch(function (e) {
			...
			var hitPct = hitDenom > 0 ? Math.round((totals.cacheRead / hitDenom) * 1000) / 10 : null;

			// 按天聚合（daily 是 天×模型 粒度 → 按天合并）：折线图序列字段；
			// 无数据的日期补零，时间轴连续铺满整个窗口（参考官方控制台趋势图）。
			// "当天"窗口改用小时级桶（00:00–23:00 补零，X 轴按小时）。
			var trend = React.useMemo(function () {
				if (days === 1 && hourly) {
					var byHour = {};
					for (var h0 = 0; h0 < hourly.length; h0++) byHour[hourly[h0].hour] = hourly[h0];
					// 小时序号习惯：hour 0-23 → X 轴 1-24（第 1~24 小时；24 = 23:00-23:59）
					// 当天视图截断到当前小时（未来时段无意义，避免右侧大片零桶显得图空）
					var outH = [];
					var currentHour = new Date().getHours();
					for (var hh = 0; hh <= currentHour; hh++) {
						var hr = byHour[hh];
						outH.push({
							// 时间点格式：hour 0-23 → 刻度 01:00~24:00（00:00-01:00 显示为 01:00）
							day: String(hh + 1).padStart(2, "0") + ":00",
							requests: hr ? hr.requests || 0 : 0,
							cost: hr ? hr.cost_usd || 0 : 0,
							input: hr ? hr.input_tokens || 0 : 0,
							output: hr ? hr.output_tokens || 0 : 0,
							cacheRead: hr ? hr.cache_read_tokens || 0 : 0,
							cacheWrite: hr ? hr.cache_write_tokens || 0 : 0,
						});
					}
					// 结合方案3：头部全零时段不画，从第一个有数据的小时起笔（中间空桶照画）
					var firstNonZero = 0;
					while (firstNonZero < outH.length) {
						var z = outH[firstNonZero];
						if (z.requests || z.cost || z.input || z.output || z.cacheRead || z.cacheWrite) break;
						firstNonZero++;
					}
					var hourlyOut = firstNonZero < outH.length ? outH.slice(firstNonZero) : outH;
					// 桶数 <15 且已拿到半小时数据：换成半小时桶加细刻度（同样截断当前 + 头部裁剪）
					if (hourlyOut.length < 15 && halfHourly) {
						var byHalf = {};
						for (var h1 = 0; h1 < halfHourly.length; h1++) {
							var row = halfHourly[h1];
							byHalf[row.hour * 2 + (row.half || 0)] = row;
						}
						var now = new Date();
						var currentBucket = now.getHours() * 2 + (now.getMinutes() >= 30 ? 1 : 0);
						var outHalf = [];
						for (var b = 0; b <= currentBucket; b++) {
							var r2 = byHalf[b];
							// 刻度 = 时段结束：前半桶 → HH:30，后半桶 → (H+1):00
							var endHour = Math.floor((b + 1) / 2), endMin = (b + 1) % 2 === 0 ? "00" : "30";
							outHalf.push({
								day: String(endHour).padStart(2, "0") + ":" + endMin,
								requests: r2 ? r2.requests || 0 : 0,
								cost: r2 ? r2.cost_usd || 0 : 0,
								input: r2 ? r2.input_tokens || 0 : 0,
								output: r2 ? r2.output_tokens || 0 : 0,
								cacheRead: r2 ? r2.cache_read_tokens || 0 : 0,
								cacheWrite: r2 ? r2.cache_write_tokens || 0 : 0,
							});
						}
						var fnz = 0;
						while (fnz < outHalf.length) {
							var z2 = outHalf[fnz];
							if (z2.requests || z2.cost || z2.input || z2.output || z2.cacheRead || z2.cacheWrite) break;
							fnz++;
						}
						return fnz < outHalf.length ? outHalf.slice(fnz) : outHalf;
					}
					return hourlyOut;
				}
				var map = {};
			...
			if (loading) {
				...
				body = h("div", { style: { display: "flex", flexDirection: "column", gap: "12px" } },
					h("div", { style: S.usageStatGrid },
						...
						h(UsageStatCard, { label: "平均 TTFT", value: fmtLatency(totals.ttft), color: STAT_COLORS.latency })),
					...
					h("div", { style: S.usageCard },
						h(ModelBarsView, { rows: allTime, providerOrder: provOrder })),
					h("div", { style: S.usageCard },
						h("div", { style: { display: "flex", alignItems: "center", gap: "2px" } },
						...
						h(RankTable, { rows: allTime, dim: rankDim })),
					h("div", { style: S.usageCard },
						h(RequestRecords, null)));
			}
