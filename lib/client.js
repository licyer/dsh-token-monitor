	...
	factory: (require) => {
		...
		function quotaText(p, now) {
			...
			if (weekly && weekly.remainingPct <= 0) {
				src = weekly; prefix = weeklyLabel + " 剩 ";
			} else if (st.window) {
				src = st.window; prefix = windowLabel + " 剩 ";
			} else if (weekly) {
				src = weekly; prefix = weeklyLabel + " 剩 ";
			} else {
				return null;
		...
		var WINDOW_OPTIONS = [1, 7, 30, 90, 0]; // 0 = "全部"（无时间上限）
		...
		/** 供应商 id → 展示名：先查 vendor 名（服务端聚合已归并），再查 provider 名（原始提供方），未映射原样返回。 */
				...
				tooltip: {
					...
					formatter: function (params) {
						...
						for (var i = 0; i < params.length; i++) {
							var p = params[i];
							var v = p.seriesName === "预估费用" ? fmtCny(p.value)
						...
						for (var i = 0; i < params.length; i++) {
							...
							if (p.seriesName === rightLabel) {
								lines.push(p.marker + " " + p.seriesName + "："
									+ (metric === "cost" ? fmtCny(p.value) : p.value + " 次"));
								break;
							}
						}
						return lines.join("<br/>");
		...
		function ModelBarsView(props) {
			...
			var TABS = [["requests", "请求次数"], ["cost", "预估费用"]];
			return h("div", null,
				h("div", { style: { display: "flex", alignItems: "center", gap: "2px" } },
					h("div", { style: S.usageCardTitle }, "供应商统计"),
					h("span", { style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" } },
						"（历史全量 · 不受筛选条件影响）"),
					h("span", { style: { flex: 1 } }),
					h("div", { style: { display: "inline-flex", gap: "4px" } },
		...
		function CalendarHeatmap(props) {
			...
			return h("div", null,
				h("div", { style: { display: "flex", alignItems: "center", gap: "2px" } },
					h("div", { style: S.usageCardTitle }, "消耗热力"),
					h("span", { style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" } },
		...
		function RequestRecords(props) {
			...
			var columns = [
				...
				{ key: "cost_usd", label: "预估费用", width: "7.5%", render: function (r) { return r.cost_usd == null ? "—" : fmtCny(r.cost_usd); } },
		...
		function RankTable(props) {
			...
			var METRIC_COLS = [
				...
				{ key: "cost_usd", label: "预估费用", width: "8.9%", render: function (r) { return fmtCny(r.cost_usd); } },
		...
		function UsageView() {
			...
			if (loading) {
				...
				body = h("div", { style: { display: "flex", alignItems: "center", gap: "10px" } },
					...
					h("button", { type: "button", style: S.refreshBtn, onClick: function () { load(days, client, provider, model, focus); } }, "重试"));
			} else {
				body = h("div", { style: { display: "flex", flexDirection: "column", gap: "12px" } },
					h("div", { style: S.usageStatGrid },
						...
						h(UsageStatCard, { label: "预估费用", value: fmtCny(totals.cost), color: STAT_COLORS.cost }),
					...
					h("div", { style: S.usageNote },
						"预估费用 = token 消耗 × pi-ai 刊例价（单位：USD，仅供参考，非实际账单；订阅制不产生真实扣费）· 已按汇率 1 USD ≈ "
							+ USDUCNY_RATE.toFixed(4)
							+ " CNY 换算"
			...
			return h("div", { style: S.usageRoot },
				h("div", { style: S.usageHeader },
					// 会话聚焦横幅：从弹层"用量详情"跳转进入；✕ 清除回到全量（客户端/时间窗保持用户选择）
					focus
						? h("span", { style: S.usageFocusBanner },
							h("span", null, "当前会话：" + (focus.title || focus.sessionId)),
							h("button", {
								type: "button", style: S.usageFocusClear, title: "取消聚焦",
