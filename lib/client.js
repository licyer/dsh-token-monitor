	...
	factory: (require) => {
		...
		var DICT = {
			zh: {
				...
				// —— 数据来源 ——
				"src.title": "数据来源",
				"src.note": "（DSH 记录支持导入/导出跨设备同步；CC 记录支持本机自动检测与 SQL 文件导入）",
				"src.col.source": "来源",
				"src.col.desc": "简要说明",
				"src.col.dir": "目录",
				"src.col.action": "操作",
				...
				"settings.version.linked": "本地开发挂载（link:）不支持在线升级",
				"settings.version.channelGithub": "GitHub 安装",
				"settings.version.channelLink": "本地挂载",
				"settings.version.channelNpm": "通过 npm 安装",
			},
			en: {
				...
				"src.note": "（DSH records: import/export for cross-device sync · CC records: local auto-detect or SQL file import）",
				...
				"settings.version.linked": "Local dev mount (link:) does not support online upgrade",
				"settings.version.channelGithub": "Installed from GitHub",
				"settings.version.channelLink": "Local mount",
				"settings.version.channelNpm": "Installed from npm",
			},
		};
		...
		function ProviderCard(props) {
			...
			return h("div", { style: S.section },
				...
				p.ok
					...
					: h("div", { style: Object.assign({}, S.error, { textAlign: "right" }) }, locServer(p.error) || t("card.queryFailed")));
		}
					...
					? h("div", { style: S.menu, role: "dialog", "aria-label": t("entry.title") },
						...
						h("div", { style: S.footer },
							...
							h("div", { style: { display: "inline-flex", alignItems: "center", gap: "8px" } },
								// 插件版本（随 overview 下发）：刷新按钮左侧，与按钮一起靠右；无版本号时不渲染
								(overview && overview.version
									? h("span", { style: { fontVariantNumeric: "tabular-nums" } },
										t("entry.version") + "v" + overview.version)
									: null),
								h("button", {
									type: "button",
		...
		function UsageView() {
			...
			if (usageFocusRequest) {
				...
				if (!focusSnapshotRef.current) {
					focusSnapshotRef.current = { client: client, provider: provider, model: model, days: days };
				}
			...
			React.useEffect(function () {
				function onFocusSession(e) {
					...
					if (!focusSnapshotRef.current) {
						focusSnapshotRef.current = { ...latestFiltersRef.current };
					}
			...
			function exportDsh() {
				...
				fetch("/token-monitor/export/dsh", { headers: { accept: "application/json" } })
					...
					.catch(function (err) { showDshResult("export", false, String((err && err.message) || err)); })
					...
					fetch("/token-monitor/import/dsh", {
						...
						.then(function (d) {
							if (d && d.ok) {
								...
								showDshResult("import", false, (d && d.error) || t("src.importDshFailed"));
							}
						})
						.catch(function (err) { showDshResult("import", false, String((err && err.message) || err)); })
						.then(function () { setDshBusy(null); });
				};
				reader.readAsText(file, "utf-8");
			}
		...
		function SettingsView() {
			...
			return h("div", { style: { display: "flex", flexDirection: "column", height: "100%" } },
				h("div", { style: { display: "flex", flexDirection: "column", flex: "1 1 auto", minHeight: 0, overflow: "auto" } },
				rows,
				...
				(function () {
					...
					// 副标题按渠道：npm 走更新判定；github/link 显示版本 + 渠道依赖原文
					var channelLabel = v.channel === "github" ? t("settings.version.channelGithub")
						: v.channel === "link" ? t("settings.version.channelLink") : "";
					var verDesc = (v.channel === "github" || v.channel === "link")
						? (v.current ? t("settings.version.current", { v: v.current }) : "")
							+ "（" + channelLabel + (v.dep ? "：" + v.dep : "") + "）"
						: v.hasUpdate
							? t("settings.version.found", { v: v.latest, c: v.current })
							: v.current
								? t("settings.version.current", { v: v.current }) + "（" + t("settings.version.latest") + "）"
					...
					return h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0", borderBottom: "1px solid var(--dsw-alias-border-l2)" } },
						...
						h("div", { style: { display: "inline-flex", alignItems: "center", gap: "8px" } },
							upgradeResult
								? h("span", { style: { fontSize: "11px", color: upgradeResult.ok ? "var(--dsw-alias-success, #34a853)" : "var(--dsw-alias-error, #ea4335)" } }, upgradeResult.text)
								: null,
							h("button", {
								type: "button",
								disabled: !v.hasUpdate || upgrading,
								// 与设置页下拉同款胶囊（高 36/圆角 18/字号 14）；有新版时换保存按钮同款主题（反色填充）
								style: Object.assign({
									display: "inline-flex", alignItems: "center", gap: "8px",
									...
									cursor: v.hasUpdate && !upgrading ? "pointer" : "default",
								}, v.hasUpdate && !upgrading
								...
								onMouseDown: function (e) { e.preventDefault(); },
								onClick: function () {
									if (!v.hasUpdate || upgrading) return;
									setUpgrading(true);
									setUpgradeResult(null);
									fetch("/token-monitor/upgrade", { method: "POST" })
										.then(function (r) { return r.json(); })
										.then(function (d) {
											if (d && d.ok) setUpgradeResult({ ok: true, text: t("settings.version.ok") });
											else if (d && d.error === "linked-dev") setUpgradeResult({ ok: false, text: t("settings.version.linked") });
											else setUpgradeResult({ ok: false, text: t("settings.version.failed") + (d && d.error ? "：" + d.error : "") });
										})
