	...
	factory: (require) => {
		...
		var DICT = {
			zh: {
				...
				// —— 图表加载 ——
				"chart.loading": "图表加载中…",
				"chart.loadFailed": "图表库加载失败（/token-monitor/vendor/echarts.min.js 不可用）",
			},
			en: {
				...
				"chart.loadFailed": "Chart library failed to load (/token-monitor/vendor/echarts.min.js unavailable)",
			},
		};
		...
		function ProviderCard(props) {
			...
			return h("div", { style: S.section },
				...
				p.ok
					...
					: h("div", { style: S.error }, p.error || t("card.queryFailed")));
		}
			...
			return providers.map(function (p) {
				...
				return h("div", { key: p.id, style: S.providerRow },
					...
					h("span", { style: p.ok ? S.rowValue : Object.assign({}, S.rowValue, S.error) },
						p.ok ? (quota || p.headline || "—") : (p.error || t("card.failed"))));
			});
		}
		...
		function TokenMonitorEntry(props) {
			...
			if (fetchError && !overview) {
				...
				label = (current ? current.model : currentProvider.label) + " · "
					+ (currentProvider.ok ? (quotaText(currentProvider, now) || currentProvider.headline || "—") : t("card.queryFailed"));
		...
		function WindowSwitcher(props) {
			return h("div", { style: { display: "inline-flex", gap: "4px" } },
				WINDOW_OPTIONS.map(function (d) {
					...
					}, d === 1 ? t("usage.windowToday") : d === 0 ? t("usage.windowAll") : t("usage.windowDays", { d: d }));
				}));
		}
