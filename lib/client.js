	...
	factory: (require) => {
		...
		var DICT = {
			zh: {
				...
				"focus.clear": "取消聚焦",
				// —— 说明文案 ——
				"note.costFormula": "预估费用 = token 用量 × 模型定价（可在下方自行维护；未维护的用 pi-ai 刊例价兜底）· 仅供参考、非实际账单；订阅制不产生真实扣费 · 已按汇率 1 USD ≈ {rate} CNY 换算",
				...
				"settings.pricing.hint": "（单价 / 百万 token，按币种列计价；只影响之后新采集的记录，已有记录的费用不会随之重算）",
			...
			en: {
				...
				// —— 说明文案 ——
				"note.costFormula": "Est. cost = token usage × model pricing (maintainable below; pi-ai list price is used as fallback) · for reference only, not an actual bill; subscriptions incur no real charge",
				...
				"settings.pricing.hint": "(priced per million tokens, in the row's currency; applies only to records collected afterwards — existing records are not recalculated)",
