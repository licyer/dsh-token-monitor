	...
	factory: (require) => {
		...
		function UsageSection(props) {
			...
			if (loading) {
				...
				body = h("div", { style: S.error }, t("card.loadFailed"));
			} else if (!usage) {
		 ...
		 * 兼容两代 DSH 客户端 API（能力探测，不依赖版本号）：
		 *  - ≤0.1.1：ctx.connection.api.sessions.models({sessionId}) → result.value.current
		 ...
		 * 任何失败/超时都回传 null，由调用方走“未识别”兜底文案。
		 */
		function resolveCurrentModel(sessionId, cb) {
			...
			// 0.1.1 及更早：connection.api.sessions.models
			var api = apiRef.current;
			...
			// 0.1.2+：remote.session（control 流首帧 baseline 即含全部会话投影）
			var rs = remoteSessionRef.current;
			if (!rs || typeof rs.control !== "function") { done(null); return; }
			...
			} catch (_e2) { settle(null); return; }

			var fallbackDefault = function () {
				if (typeof rs.modelCatalog !== "function") { settle(null); return; }
			...
			var read = function () {
				Promise.resolve().then(function () { return iterator.next(); }).then(function (step) {
					if (step.done) { settle(null); return; }
			...
			read();
			// 安全网：连接未就绪等导致 baseline 迟迟不来时兜底（10s）
		...
		function SettingsView() {
			...
			return h("div", { style: { display: "flex", flexDirection: "column", height: "100%" } },
				...
				(function () {
					...
					// 副标题：link 显示版本 + 渠道依赖原文（不提示更新）；unknown 恒显示
					// "当前版本（已是最新版本）"（有无新版本都不变）；npm/github 按 hasUpdate 判定
					var verDesc;
					if (v.channel === "link") {
						...
						verDesc = v.hasUpdate
							...
							: (v.current ? t("settings.version.current", { v: v.current }) + "（" + t("settings.version.latest") + "）" : t("settings.version.latest"));
					} else {
								...
								onClick: function () {
									...
									fetch("/token-monitor/upgrade", { method: "POST" })
										...
										.then(function (d) {
											...
											else setUpgradeResult({ ok: false, text: t("settings.version.failed") });
										})
		...
		function apply(ctx) {
			...
			// DSH 0.1.2+：探测类型化 Remote 命名空间 remote.session（老版本没有该
			// 服务，ctx.inject 永不回调 → remoteSessionRef 保持 null，走 legacy 通道）。
