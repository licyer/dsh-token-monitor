...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var USAGE_URLS = {
			...
			heatmap: "/token-monitor/usage/heatmap",
		};
		...
		var TREND_SERIES = [
			{ key: "cost", label: "费用", color: "#e5484d", axis: "cost", dashed: true },
			// 总输入 = 新增输入 + 缓存命中 + 缓存创建（供应商计费口径的总入侧）
			{ key: "totalInput", label: "总输入", color: "#1a73e8", axis: "token",
				get: function (d) { return (d.input || 0) + (d.cacheRead || 0) + (d.cacheWrite || 0); } },
			{ key: "cacheRead", label: "缓存命中", color: "#8e4ec6", axis: "token", fill: true },
			{ key: "output", label: "输出", color: "#34a853", axis: "token" },
		];
		...
		/** 趋势图 option：token 四构成走左轴，成本走右轴（红色虚线），缓存命中带面积填充。 */
		function buildTrendOption(data) {
			...
			var showSymbol = data.length <= 45;
			return {
				animationDuration: 300,
				// containLabel: 轴标签宽度计入 grid，窗口窄时标签也不会被裁掉
				grid: { left: 12, right: 12, top: 16, bottom: 54, containLabel: true },
				legend: {
					// 图例符号：circle 圆点；可选 rect/roundRect/triangle/diamond/pin/arrow，
					// 或 path:///image:// 自定义，也可在 legend.data 里按序列分别指定
					...
					formatter: function (params) {
						...
						for (var i = 0; i < params.length; i++) {
							...
							lines.push(p.marker + " " + p.seriesName + "："
								+ (p.seriesName === "费用" ? fmtUsd(p.value) : fmtTokens(p.value)));
						}
				...
				xAxis: {
					...
					// 小时桶（"14:00"）原样显示；日期桶截掉年份
					axisLabel: { fontSize: 10, color: axisColor, formatter: function (v) { var s = String(v); return s.indexOf(":") >= 0 ? s : s.slice(5); } },
				...
				yAxis: [
					...
					},
					{
						...
						axisLabel: { fontSize: 10, color: axisColor, formatter: fmtAxisUsd },
						axisLine: { show: true, lineStyle: { color: gridColor } },
						axisTick: { show: true, lineStyle: { color: gridColor } },
						splitLine: { show: false },
					},
				],
				series: TREND_SERIES.map(function (s) {
					var item = {
						...
						data: data.map(s.get || function (d) { return d[s.key] || 0; }),
					};
					if (s.fill) item.areaStyle = { color: s.color, opacity: 0.08 };
					return item;
				}),
			};
		}
		...
		function DailyTrend(props) {
			...
			var option = React.useMemo(function () {
				return data.length ? buildTrendOption(data) : null;
			}, [serialized]);
			if (!data.length) return h("div", { style: S.muted }, "窗口内暂无数据");
			return h(EChart, { option: option, serialized: serialized, height: 300 });
		}

		/* ---- 时段热力图：小时 × 星期几（调用次数，echarts heatmap） ---- */

		var HEAT_DOW_LABELS = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"];
		var HEAT_HOURS = [];
		for (var hi = 0; hi < 24; hi++) HEAT_HOURS.push(String(hi).padStart(2, "0"));

		function buildHeatmapOption(rows) {
			var axisColor = cssVar("--dsw-alias-label-tertiary", "#9aa0a6");
			var brand = cssVar("--dsw-alias-brand-primary", "#3964fe");
			var pageBg = cssVar("--dsw-alias-bg-base", "#ffffff");
			var cells = [];
			var max = 0;
			for (var i = 0; i < rows.length; i++) {
				...
				var v = r.requests || 0;
				if (v > max) max = v;
				// dow: sqlite %w 0=周日..6=周六 → 行序改为周一..周日
				cells.push([r.hour, (r.dow + 6) % 7, v, r.tokens || 0]);
			}
			return {
				animationDuration: 300,
				...
				tooltip: {
					formatter: function (p) {
						var d = p.data;
						return HEAT_DOW_LABELS[d[1]] + " " + HEAT_HOURS[d[0]] + ":00 · "
							+ d[2] + " 次调用 · " + fmtTokens(d[3]) + " tokens";
					},
				},
				xAxis: {
					type: "category", data: HEAT_HOURS,
					axisLabel: { fontSize: 10, color: axisColor, interval: 3 },
					axisLine: { show: false }, axisTick: { show: false },
				},
				yAxis: {
					type: "category", data: HEAT_DOW_LABELS,
					axisLabel: { fontSize: 10, color: axisColor },
					axisLine: { show: false }, axisTick: { show: false },
				},
				visualMap: {
					min: 0, max: max || 1, calculable: false,
					orient: "horizontal", right: 8, top: 2, itemWidth: 10, itemHeight: 90,
					text: ["多", "少"], textStyle: { fontSize: 10, color: axisColor },
					inRange: { color: ["rgba(128,128,128,0.10)", brand] },
				},
				series: [{
					type: "heatmap", data: cells,
					label: { show: false },
					itemStyle: { borderColor: pageBg, borderWidth: 2, borderRadius: 2 },
					emphasis: { itemStyle: { shadowBlur: 6, shadowColor: "rgba(0,0,0,0.25)" } },
				}],
			};
		}

		/** 时段热力图：props.data = [{ hour, dow, requests, tokens }]。 */
		function UsageHeatmap(props) {
			...
			if (!rows.length) return h("div", { style: S.muted }, "窗口内暂无数据");
			return h(EChart, { option: option, serialized: serialized, height: 220 });
		}
		...
		function FilterSelect(props) {
			...
			var label = current ? current.label : (props.placeholder || "全部");

			return h("div", { ref: rootRef, style: { position: "relative", display: "inline-flex" } },
				h("button", {
					type: "button",
					style: Object.assign({}, props.style || S.windowBtn, {
						display: "inline-flex",
				...
				open
					? h("ul", { style: Object.assign({}, S.filterMenu, { minWidth: props.menuWidth || "110px" }), role: "listbox" },
						opts.map(function (o) {
		...
		function UsageView() {
			...
			// 时段热力图数据（小时 × 星期）
			var _heat = React.useState(null), heat = _heat[0], setHeat = _heat[1];
			...
			var load = React.useCallback(function (d) {
				...
				var jobs = [
					...
					fetch(USAGE_URLS.heatmap + q, { headers: { accept: "application/json" } }).then(ok),
				];
				// "当天"额外拉小时级趋势
				if (d === 1) jobs.push(fetch(USAGE_URLS.hourly + "?day=" + localDay(Date.now()), { headers: { accept: "application/json" } }).then(ok));
				Promise.all(jobs).then(function (results) {
					setDaily(results[0].data || []);
					setByModel(results[1].data || []);
					setSessions(results[2].data || []);
					setHeat(results[3].data || []);
					setHourly(d === 1 && results[4] ? (results[4].data || []) : null);
				}).catch(function (e) {
			...
			// 大数字卡汇总：总消耗 / 调用次数 / 费用 / TTFT + 第二排 token 构成（新增输入/缓存命中/输出）
			...
			// 按供应商汇总（byModel 行自带 provider 维度，客户端聚合即可，零额外查询）
			var providerCards = React.useMemo(function () {
				var map = {};
				var rows = byModel || [];
				for (var i = 0; i < rows.length; i++) {
					var r = rows[i];
					var p = map[r.provider];
					if (!p) p = map[r.provider] = { provider: r.provider, requests: 0, cost: 0, input: 0, output: 0, cacheRead: 0 };
					p.requests += r.requests || 0;
					p.cost += r.cost_usd || 0;
					p.input += r.input_tokens || 0;
					p.output += r.output_tokens || 0;
					p.cacheRead += r.cache_read_tokens || 0;
				}
			...
			var trend = React.useMemo(function () {
				if (days === 1 && hourly) {
					...
					var p0 = function (x) { return String(x).padStart(2, "0"); };
					var outH = [];
				...
				for (var i = 0; i < rows.length; i++) {
					var r = rows[i];
					var d = map[r.day];
					if (!d) d = map[r.day] = { day: r.day, cost: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0 };
					d.cost += r.cost_usd || 0;
				...
				for (var i2 = days - 1; i2 >= 0; i2--) {
					...
					out.push(map[key] || { day: key, cost: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0 });
				}
				return out;
			}, [daily, hourly, days]);
			...
			var filterOptions = React.useMemo(function () {
				...
				return {
					clients: clientIds.sort().map(function (id) { return { id: id, label: id }; }),
					providers: providerIds.sort().map(function (id) { return { id: id, label: id }; }),
					models: modelIds.sort().map(function (id) { return { id: id, label: id }; }),
				};
			}, [byModel, client, provider]);

			// 按筛选条件过滤排行行（by-model 行含 client/provider/model 字段）
			var filteredByModel = React.useMemo(function () {
				var rows = byModel || [];
			...
			if (loading) {
				...
				body = h("div", { style: { display: "flex", flexDirection: "column", gap: "12px" } },
					...
					// 第二排：token 构成（新增输入 / 缓存命中 / 输出）+ 缓存命中率（带进度条）
					h("div", { style: S.usageStatGrid },
						h(UsageStatCard, { label: "新增输入", value: fmtTokens(totals.input) }),
						h(UsageStatCard, { label: "缓存命中", value: fmtTokens(totals.cacheRead) }),
						h(UsageStatCard, { label: "输出", value: fmtTokens(totals.output) }),
						hitPct !== null
							? h(UsageStatCard, { label: "缓存命中率", value: hitPct + "%" })
							: h("div", { style: { minHeight: "44px" } })),
					h("div", { style: S.usageNote },
						totals.unpriced > 0
							? "另有 " + totals.unpriced + " 次调用未定价（计入 token、不计入费用）"
							: "全部调用均已按 pi-ai 刊例价定价 · 费用单位 USD"),
					// 第三排：按供应商汇总卡组（费用大数字 + tokens/调用/缓存命中率副行）
					providerCards.length
						? h("div", { style: S.usageStatGrid },
							providerCards.map(function (p) {
								var pTokens = p.input + p.output + p.cacheRead;
								var pHitDenom = p.cacheRead + p.input;
								var pHitPct = pHitDenom > 0 ? Math.round((p.cacheRead / pHitDenom) * 1000) / 10 : null;
								return h("div", { key: p.provider, style: S.usageStatCard },
									h("span", { style: S.usageStatLabel }, "供应商 · " + p.provider),
									h("span", { style: Object.assign({}, S.usageStatValue, { color: "var(--dsw-alias-state-business-primary, #1a73e8)" }) }, fmtUsd(p.cost)),
									h("div", { style: { display: "flex", flexWrap: "wrap", gap: "1px 12px", fontSize: "11px", color: "var(--dsw-alias-label-tertiary)" } },
										h("span", null, "Tokens " + fmtTokens(pTokens)),
										h("span", null, "调用 " + fmtTokens(p.requests)),
										pHitPct !== null ? h("span", null, "缓存 " + pHitPct + "%") : null));
							}))
						: null,
					h(UsageCard, { title: "使用趋势" },
						h(DailyTrend, { data: trend })),
					h(UsageCard, { title: "时段热力图（调用次数 · 小时 × 星期）" },
						h(UsageHeatmap, { data: heat })),
					h(UsageCard, { title: "按模型排行" },
						filteredByModel.length
							? h(UsageTable, { columns: modelColumns, rows: filteredByModel })
							: h("div", { style: S.muted }, "该筛选条件下暂无数据")),
			...
			return h("div", { style: S.usageRoot },
				h("div", { style: S.usageHeader },
					...
					h("button", {
						// 与窗口切换标签同高同字号（refreshBtn 默认尺寸是给弹层 11px 语境的）
						type: "button",
