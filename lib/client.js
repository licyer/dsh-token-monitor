...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var PROVIDER_URLS = {
			...
			zhipu: "https://open.bigmodel.cn/coding-plan/personal/overview",
			"zai-coding-cn": "https://open.bigmodel.cn/coding-plan/personal/overview",
			zai: "https://open.bigmodel.cn/coding-plan/personal/overview",
			...
			return {
				...
				tooltip: {
					...
					formatter: function (params) {
						...
						var lines = [trendRangeLabel(params[0].axisValue, stepMin)];
		...
		function DailyTrend(props) {
			...
			var stepMin = data.step || 0;
			var serialized = JSON.stringify({ d: data, m: metric, l: LANG, s: stepMin });
					...
					formatter: function (params) {
						...
						if (vms && vms.length) {
							for (var vi = 0; vi < vms.length; vi++) {
								...
								lines.push('<span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:' + modelBarColor(vendorId, shade2) + ';margin-right:4px"></span>'
		...
		/** 蓝系六级色阶：零值 GitHub 灰 + 非零 5 档（首档明显偏蓝，与零值一眼区分）。 */
			...
			return {
				...
				visualMap: {
					...
					pieces: [
						...
						{ gt: 0, lte: t1, color: HEAT_COLORS[1] },
						{ gt: t1, lte: t2, color: HEAT_COLORS[2] },
						{ gt: t2, lte: t3, color: HEAT_COLORS[3] },
						{ gt: t3, lte: t4, color: HEAT_COLORS[4] },
						{ gt: t4, color: HEAT_COLORS[5] },
					],
				},
				calendar: {
