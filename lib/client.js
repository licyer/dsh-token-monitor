...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 徽标文字变色阈值（点已移除：颜色直接落在余量/余额文字上）。
		 * **只有一档：红 = 告急**。正常一律用默认字色（不染绿、不染橙）——
		 * 绿/橙常态着色会让"余量还够"也一直抢视觉注意力。
		 * - 订阅额度（quota）：红色档看**显示窗口**是否见底（剩余 ≤0）；7d 剩余低于
		 *   `BADGE_WEEKLY_ERR_PCT` 也一并报红（周额度快用完，比 5h 更值得提醒）。
		 * - 按量余额（balance）：余额 ≤ `BADGE_BALANCE_ERR` 报红，按各自币种的**原值**判。
		 */
		var BADGE_BALANCE_ERR = 2;       // 余额 ≤ 此值 → 红
		var BADGE_WEEKLY_ERR_PCT = 10;   // 7d 窗口剩余 ≤ 此值 → 红
		...
		/** 官方 sessions 客户端服务句柄（binding() 服务，见 service.d.ts）；供 resolveCurrentModel 取模型。 */
		var sessionsRef = { current: null };
		/** DSH 主题服务句柄（dsh-client-ui-theme；apply 时经 ctx.get 取、缺失则由 ctx.inject 补）。 */
		var themeServiceRef = { current: null };
		/** slots 服务句柄：账号渠道充值按钮探测/触发官方平台页面（shell.overlay 条目）用。 */
		var slotsServiceRef = { current: null };

		/**
		 * 当前是否为深色外观，三级判定：
		 *  1. theme 服务 getTheme().active.colorScheme —— 权威（已解析 system 偏好）；
		 *  2. body[data-ds-dark-theme] —— 宿主打在 body 上的暗色标记（设计令牌的暗色分支
		 *     就写在这个选择器里），比"量背景亮度"可靠：body 的背景在新版布局里可能仍是
		 *     浅色/透明，光靠亮度会在暗色主题下判成 light；
		 *  3. 兜底：读 body 计算背景的相对亮度（<0.5 判深色）。
		 */
		function isDark() {
		...
		/**
		 * 需要框架保证就绪的服务（Cordis fiber inject = 硬闸门：任一服务缺失即
		 * epoch=INACTIVE → 永不 apply，且**没有超时**）。
		 * 因此这里只放**新旧版都提供**的基础服务；0.1.7 才有的 sessions / remote
		 * 一律不进 inject，改用 ctx.get + ctx.inject 双路探测（见 apply 内），
		 * 缺服务时静默回退旧通道——否则旧版 DSH 上插件会彻底不启动。
		 */
		var inject = ["slots", "connection", "locale", "theme"];
		...
		var DICT = {
			zh: {
				...
				"focus.clear": "取消聚焦",
				// —— 说明文案 ——
				"note.costFormula": "预估费用 = token 用量 × 模型定价（可在下方自行维护）· 仅供参考、非实际账单 · 当前汇率 1 USD ≈ {rate} CNY",
				// —— 供应商展示名 ——
				"vendor.unknown": "未知",
				"vendor.kimi": "Kimi",
				"vendor.zhipu": "智谱（GLM）",
				...
				"settings.pricing.hint": "（单价 / 百万 token；未维护的用 pi-ai 刊例价兜底；只对新记录生效，历史费用不重算）",
			...
			en: {
				...
				// —— 聚焦横幅 ——
				"focus.current": "Current session",
				"focus.clear": "Clear focus",
				// —— 说明文案 ——
				"note.costFormula": "Est. cost = token usage × model pricing (maintainable below) · for reference only, not an actual bill · current rate 1 USD ≈ {rate} CNY",
				// —— 供应商展示名 ——
				...
				"settings.pricing.hint": "(priced per million tokens; pi-ai list price is used as fallback; applies only to new records; existing costs are not recalculated)",
		...
		/**
		 * 从服务端指标的展示值里取金额（按量余额类用）。
		 * 抓取器给的 value 形如 `49.59 CNY` / `$12.50`；**必须是纯数字**（可带千分位）
		 * 才算金额——`78%`（百分比额度）、`—`、`端点可用…` 都返回 null。
		 * 币种只用于展示：阈值按各自币种的**原值**判，不做汇率换算（换算会把"剩 8 元"
		 * 变成"剩 1.1 美元"，阈值语义随之漂移）。
		 */
		function balanceAmountOf(value) {
			var s = String(value == null ? "" : value).trim();
			// 币种符号可前可后：`49.59 CNY` / `$12.50` / `¥8.00` 都要认；
			// 百分比（`78%`）因尾部不是币种而对不上，天然被排除。
			var m = s.match(/^(?:USD|CNY|美元|人民币|\$|¥|￥)?\s*(-?\d[\d,]*(?:\.\d+)?)\s*(?:USD|CNY|美元|元|\$|¥|￥)?$/i);
			if (!m) return null;
			var n = Number(m[1].replace(/,/g, ""));
			return isFinite(n) ? n : null;
		}

		/** 纯数字（可带千分位）→ 百分比额度：MiniMax / 智谱等只返回 metrics 的订阅类，
		 *  value 形如 `78%`（已用百分比，与服务端 MetricRow 同刻度）。 */
		function usedPctOf(value) {
		...
		/**
		 * 徽标文字的状态色：**只有"告急红"一档**，正常/无数据一律用默认字色。
		 * 按三种数据形态分流，顺序即优先级：
		 *  1. `stats`（抓取器给了结构化窗口：Kimi / OpenCode / Command Code）——
		 *     跟随**当前显示的那个窗口**的剩余百分比，与 `quotaText` 的优先级逐字同源
		 *     （7d 见底 → 5h → 7d），避免"显示 5h 的数、染 7d 的色"。
		 *  2. `metrics` 里的百分比值（MiniMax / 智谱：只有 metrics、没有 stats）——
		 *     按**已用%** 同刻度判（>95% 报红）；这类拿不到窗口剩余语义，用已用% 是
		 *     同一把尺子的另一头。注意 `78%` 必须先被识别成百分比，否则会被当成金额。
		 *  3. `metrics` 里的金额（DeepSeek / Moonshot / OpenRouter）——≤ 阈值报红。
		 * 未配置凭证 / 未适配 / 加载中一律灰（muted）：不能落进告警色，
		 * 否则"没配 key"看着像"余额告警"。返回 "none" = 用默认字色。
		 */
		function badgeTextState(p) {
			if (p && p.unsupported) return "muted";
			if (!p || !p.ok) {
				if (!p || p.configured === false) return "muted";
				return "err";
			}
			var st = p.stats;
			if (st && (st.window || st.weekly || st.monthly)) {
				// 与 quotaText 同源：长周期 = weekly（旧体系 7d）优先，新体系无 7d 用 monthly（30d）；
				// 5h 耗尽且长周期也耗尽（双重耗尽）才切换长周期窗口染色
				var weekly = st.weekly;
				var monthly = st.monthly;
				var longTerm = weekly || monthly;
				var src;
				if (st.window) {
					src = st.window;
					if (src.remainingPct <= 0 && longTerm && longTerm.remainingPct <= 0) src = longTerm;
				} else {
					src = longTerm;
				}
				// 已过重置时刻的窗口按满额 100% 计（徽标已显示 100%，颜色不应报红）；
				// 刷新拿到真实新值后自然重渲染
				var effRemain = function (s) {
					if (!s) return null;
					if (s.resetAt && s.resetAt - Date.now() <= 0) return 100;
					return s.remainingPct;
				};
				var remain = effRemain(src);
				if (typeof remain !== "number") return "none";
				if (remain <= 0) return "err";
				var longRemain = effRemain(longTerm);
				if (longRemain !== null && longRemain <= BADGE_WEEKLY_ERR_PCT) return "err";
				return "none";
			}
			var metrics = p.metrics || [];
			// 先看百分比（订阅类只有 metrics 的情形）：有窗口百分比就说明这是额度而非余额
			for (var i = 0; i < metrics.length; i++) {
				var used = usedPctOf(metrics[i] && metrics[i].value);
				if (used === null) continue;
				return used > 95 ? "err" : "none";
			}
			// 再按金额判。先按标签找余额行，找不到再扫全部（>0 的最小值）：
			// 各抓取器的标签并不统一（DeepSeek 是「人民币账户 / CNY 账户」，
			// Moonshot 是「可用余额」，OpenRouter 是「积分余额」，
			// 后端在账户无余额时还会退化成「可用 / 是|否」），单靠标签匹配会漏。
			var amt = null;
			for (var j = 0; j < metrics.length; j++) {
				if (/余额|balance|账户|account/i.test(String(metrics[j] && metrics[j].label || ""))) {
					amt = balanceAmountOf(metrics[j].value);
					if (amt !== null) break;
				}
			}
			if (amt === null) {
				for (var k = 0; k < metrics.length; k++) {
					var n = balanceAmountOf(metrics[k] && metrics[k].value);
					if (n === null || n <= 0) continue;
					if (amt === null || n < amt) amt = n;
				}
			}
			if (amt === null) return "none";
			// 动态阈值：账号渠道（DeepSeek Account）携带用户自己在平台设置的余额预警线
			// （fetcher 从 /users/current 的 balance_alert 读取，balanceAlert.{enabled,bound}）；
			// 其余渠道维持固定 BADGE_BALANCE_ERR。
			var threshold = BADGE_BALANCE_ERR;
			var ba = p && p.balanceAlert;
			if (ba && ba.enabled === true && typeof ba.bound === "number" && isFinite(ba.bound)) {
				threshold = ba.bound;
			}
			return amt <= threshold ? "err" : "none";
		}

		/* ------------------------------ 样式 ------------------------------ */

		/**
		 * 日历热力图"格子间隙"的颜色（= 日格子底层填充/描边，盖住默认浅色底透出的线条）。
		 *
		 * 必须是**渲染期同步取**，不能走 state+effect 实测：早前实现是挂载后
		 * `setBg(computedBg(...))` 存进 state，切主题时 CSS 变量同一帧就换了（卡片整体
		 * 立刻变色），而 state 要等 effect 跑完才更新 —— 那一帧 ECharts 仍用旧主题的
		 * 实测色画间隙，下一帧才换新色，看起来就是"间隙闪一下"。
		 * 优先读主题令牌（随主题即时解析，与卡片底色同源同帧）；拿不到再退回旧的实测法。
		 */
		function heatGapColor(el) {
			var v = cssVar("--dsw-alias-bg-base", "");
			if (v) return v;
			return el ? computedBg(el) : (isDark() ? "#151517" : "#ffffff");
		}

		/**
		 * 浮层实底（弹层 / 下拉 / 气泡 / tooltip 通用）：0.1.7 起
		 * `--dsw-specific-menu` = `--dsw-menu-surface-fill`，是**半透明**色
		 * （亮 α≈58% / 暗 α≈45%），必须同时给 `--dsw-menu-backdrop-filter`
		 * （blur(40px) saturate(150%)）才会显实底——只给 background 不给滤镜，
		 * 浮层就会透出下层内容（下拉/tooltip 看着"透明"）。
		 * 旧版无该滤镜令牌：var(..., none) 回退 none（无滤镜，维持旧版观感）。
		 * 新增浮层请复用它，别再只写 background。
		 */
		var MENU_SURFACE = {
			background: "var(--dsw-specific-menu)",
			backdropFilter: "var(--dsw-menu-backdrop-filter, none)",
			WebkitBackdropFilter: "var(--dsw-menu-backdrop-filter, none)",
		};
		/** tooltip（ECharts 内部 DOM）用的同款实底片段：ECharts 只收 CSS 文本。 */
		var MENU_SURFACE_CSS =
			"backdrop-filter:var(--dsw-menu-backdrop-filter,none);-webkit-backdrop-filter:var(--dsw-menu-backdrop-filter,none);";

		var S = {
			// 结构照官方 open-in-app 的 .CAgGvG_split / .CAgGvG_main：
			//   外层胶囊（边框/圆角/overflow:hidden + align-items:stretch）
			//     └ 内层 button（无边框/无圆角，被拉伸填满，自己 align-items:center 居中内容）
			// 为什么必须这样分两层：官方那套里"边框盒子"与"居中盒子"是同一个（stretch 后
			// 内层高度 = 外层内容高度，且 border-box 的 28px 里只有 2×0.5px 边框被外层吃掉），
			// 内层是整数内容盒，居中不会有取整误差；而"边框直接长在按钮上 + 自己带 padding"
			// 时，内容盒高度是 28−1−10=17 这种奇数，再叠上行盒基线，移动端就会看出偏。
			// 文本无法像图标那样"固定高度 + 两端等分"，所以额外在 label 上钉住 line-height，
			// 其余全部交给 flex 居中。
			root: { position: "relative", display: "inline-flex" },
			pill: {
				height: "28px", boxSizing: "border-box",
				color: "var(--dsw-alias-label-primary)",
				background: "none", border: "0.5px solid var(--dsw-alias-border-l4)",
				borderRadius: "14px", alignItems: "stretch", overflow: "hidden",
				fontFamily: "var(--dsw-font-family)",
				display: "inline-flex", whiteSpace: "nowrap",
			},
			// 内层按钮：与官方 .CAgGvG_main 同规格（padding/gap/字号/行高一致），
			// 但它是 stretch 项、没有边框，内容由 align-items:center 居中。
			trigger: {
				border: 0, background: "none", cursor: "pointer", color: "inherit",
				fontFamily: "inherit", fontSize: "11px", lineHeight: "16px",
				alignItems: "center", justifyContent: "center",
				gap: "5px", padding: "5px 6px 5px 7px",
				display: "inline-flex", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap",
			},
			dot: function (state) {
				// 变量用主题里**真实存在**的名字：--dsw-alias-success/warning/error 与
				// --dsw-alias-label-quaternary 在 DSH 主题里都没有定义，一直靠写死的
				// fallback 顶着（不跟随主题）；官方真名是 --dsw-alias-state-*-primary，
				// 弱化态用 --dsw-alias-label-tertiary（与 stateColor 的 muted 同色）。
				var color = state === "ok" ? "var(--dsw-alias-state-success-primary, #34a853)"
					: state === "warn" ? "var(--dsw-alias-state-warn-primary, #f9ab00)"
					: state === "err" ? "var(--dsw-alias-state-error-primary, #ea4335)"
			...
			menu: {
				...
				// 注意：这是**桌面**口径（徽标在头部 ~58px 处）。移动端头部更高（实测徽标下边
				// 85.5px），且徽标在工具条里靠左，absolute+right:0 会让 340px 的面板从徽标
				// 右边缘向左长出屏幕（实测左出屏 48.5px）——移动端由 .tm-menu 的媒体查询
				// 覆盖成 position:fixed 左右各 16px（见注入样式处）。
				maxHeight: "calc(100vh - 110px)", overflow: "auto",
				boxSizing: "border-box", border: "1px solid var(--dsw-alias-border-l2)",
				...MENU_SURFACE, borderRadius: "12px",
			...
			popMenu: {
				...
				...MENU_SURFACE, border: "1px solid var(--dsw-alias-border-l2)",
			...
			stateColor: function (state) {
				// "none" = 徽标的"正常"档：用默认主字色（不染绿/橙，只有告急才红）。
				// 注意必须显式处理——否则会落进下面 ok 分支变成绿色（曾踩过）。
				if (state === "none") return "var(--dsw-alias-label-primary)";
				// "muted"（未配置凭证 / 未适配 / 加载中）用次级弱化字色。
				// 变量名必须是主题里**真实存在**的：原先写的 --dsw-alias-label-quaternary
				// 在 DSH 主题里不存在，带 fallback 的 var() 在未定义时整条声明作废
				// （invalid at computed-value time），文字会落回按钮的 UA 默认色而非变灰。
				if (state === "muted" || state === "idle") return "var(--dsw-alias-label-tertiary)";
				// ok/warn 仍给弹层进度条与统计瓦片用（徽标不再走这两档）
				return state === "err" ? "var(--dsw-alias-state-error-primary, #ea4335)"
					: state === "warn" ? "var(--dsw-alias-state-warn-primary, #f9ab00)"
			...
			// 数据来源表格操作列图标按钮：带框（同 sourceBtn 语言），inline-flex 保证图标
			// 在框内上下垂直居中（不受行高/基线影响）；padding 收窄使图标按钮近似正方形；
			// 图标默认 tertiary（安静不抢眼），hover 样式见 tm-source-icon-btn 注入的 CSS。
			sourceIconBtn: {
				background: "none", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: "5px",
				padding: "1px 6px", cursor: "pointer", fontSize: "11px", lineHeight: "16px",
			...
			// 用量页视图页签：与 windowBtn 同语言，选中态用中性底 + 主字色，区分“视图切换”。
			// 不加底部分割线：页签条与下方面板之间已有容器 12px 间距，再加线显得多余。
			usageTabs: { display: "flex", flexWrap: "wrap", gap: "4px" },
			usageTab: {
				background: "none", borderWidth: "1px", borderStyle: "solid",
				borderColor: "transparent", borderRadius: "6px", padding: "4px 12px",
				fontSize: "12px", lineHeight: "18px", cursor: "pointer",
				color: "var(--dsw-alias-label-secondary)", boxSizing: "border-box",
				display: "inline-flex", alignItems: "center", justifyContent: "center",
			},
			usageTabActive: {
		...
		/**
		 * 徽标额度文案。规则（双重耗尽才切换长周期）：
		 *  1. 5h 有余量 → "5h 余 X%"
		 *  2. 5h 耗尽、长周期还有剩余 → 仍显示 "5h 余 0%"（带 5h 重置倒计时，用户关心何时恢复）
		 *  3. 5h 耗尽 且 长周期（本月/7d）也耗尽 → 显示长周期告急（"本月 余 0%" / "7d 余 0%"）
		 *  4. 无 5h 数据 → 降级显示长周期
		 * 长周期槽位：旧体系 7d（weekly）优先；新体系（Kimi 新套餐）无 7d，用月度（monthly）。
		 */
		function quotaText(p, now, short) {
			var st = p && p.stats;
			if (!st) return null;
			var weekly = st.weekly;
			var monthly = st.monthly;
			var longTerm = weekly || monthly;
			// 窗口展示名：供应商 stats.labels 提供（opencode：滚动/周/月）；缺省 Kimi 语义 5h/7d/本月
			var lab = st.labels || {};
			var windowLabel = lab.window || "5h";
			var weeklyLabel = lab.weekly || "7d";
			var monthlyLabel = lab.monthly || "30d";
			var longLabel = weekly ? weeklyLabel : monthlyLabel;
			var src;
			var prefix;
			if (st.window) {
				src = st.window; prefix = t("card.remaining", { label: windowLabel });
				// 双重耗尽才切换长周期告急
				if (src.remainingPct <= 0 && longTerm && longTerm.remainingPct <= 0) {
					src = longTerm; prefix = t("card.remaining", { label: longLabel });
				}
			} else if (longTerm) {
				src = longTerm; prefix = t("card.remaining", { label: longLabel });
			} else {
			...
			if (isFinite(ms) && ms <= 0) {
				// 已过重置时刻：按新窗口满额显示（反正刷新完就是 100%，等真实数据回来重渲染），
				// 不再显示"正在刷新…"占位
				return prefix + "100%";
			}
			// short=true（移动端徽标）：不拼倒计时，只留"5h 余 99%"
			var cd = "";
			if (!short) {
		...
		function MetricRow(props) {
			...
			// detailLeft：detail 行左侧内容（如账号渠道的"累计消费"），右侧保持 detail；
			// 有 detailLeft 时该行改为两端分布
			if (props.detail || props.detailLeft) {
				children.push(props.detailLeft
					? h("div", { key: "detail", style: Object.assign({}, S.rowDetail, { display: "flex", textAlign: undefined }) },
						h("span", null, props.detailLeft),
						h("span", { style: { marginLeft: "auto" } }, props.detail))
					: h("div", { key: "detail", style: S.rowDetail }, props.detail));
			}
		...
		/* ---- DeepSeek 账号渠道：原生充值入口（桌面版独有） ----
		 * 设计原则：与"自己配置 key"卡片同款胶囊按钮，唯一不同是点击动作——
		 * 不重画页面，而是**触发官方平台页面流程**：从 slots 的 shell.overlay 里找到
		 * 官方账号插件注册的 "account.platform-page" 条目，调其 inject() 拿到
		 * platformPages store，调 store.open("top-up") —— 官方 AccountPlatformHost /
		 * PlatformOverlay 接管渲染（返回栏、原生 webview、边界跟随全是官方实现），
		 * 与设置页"账号与余额 → 充值"完全同源，永远同步。
		 * web 端无该条目（官方仅在 nativePlatform 存在时注册）→ 按钮不渲染。 */
		function accountPlatformPages() {
			var slots = slotsServiceRef.current;
			if (!slots || typeof slots.entries !== "function") return null;
			var list;
			try { list = slots.entries("shell.overlay"); } catch (_e) { return null; }
			for (var i = 0; i < (list || []).length; i++) {
				var e = list[i];
				if (!e || !e.options || e.options.id !== "account.platform-page" || typeof e.inject !== "function") continue;
				try {
					var props = e.inject();
					var pages = props && props.hooks && props.hooks.page;
					if (pages && typeof pages.open === "function") return pages;
				} catch (_e2) { /* 忽略 */ }
			}
			return null;
		}
		function AccountTopUpButton() {
			var _h = React.useState(false), hovered = _h[0], setHovered = _h[1];
			// web 端（无桌面账号平台页面条目）不渲染
			if (!accountPlatformPages()) return null;
			return h("button", {
				type: "button",
				title: t("entry.openSite"),
				style: Object.assign({ border: 0, cursor: "pointer", fontFamily: "inherit" },
					hovered
						? Object.assign({}, S.externalLink, { color: "var(--dsw-alias-state-business-primary, #1a73e8)", background: "rgba(26,115,232,0.10)" })
						: S.externalLink),
				onMouseEnter: function () { setHovered(true); },
				onMouseLeave: function () { setHovered(false); },
				onClick: function () {
					var pages = accountPlatformPages();
					// open(page, onClose)：官方流程接管；onClose 无需动作（弹层数据随轮询刷新）
					if (pages) pages.open("top-up", function () { });
				},
			}, t("entry.topUp"));
		}
		...
		function ProviderCard(props) {
			...
			return h("div", { style: S.section },
				h("div", { style: S.sectionTitle },
					...
					h("span", { style: { display: "inline-flex", alignItems: "center", gap: "6px" } },
						h("span", null, p.label),
						// 账号渠道（桌面版登录）：充值按钮绑定桌面版原生充值动作；
						// 其余渠道走官网外链。web 端无 dshPlatform，AccountTopUpButton 不渲染
						p.id === "deepseek-account"
							? h(AccountTopUpButton, null)
							: h(ProviderSiteLink, { url: providerUrl(p.id, p.kind), kind: p.kind })),
					h("span", { style: { flex: 1 } }),
					h("span", { style: S.muted }, [p.badge, props.model && props.model.model].filter(Boolean).join(" · "))),
				p.ok
					? metrics.map(function (m, i) { return h(MetricRow, { key: i, label: locServer(m.label), value: locServer(m.value), detail: locServer(m.detail), detailLeft: m.detailLeft ? locServer(m.detailLeft) : undefined, pct: m.pct }); })
		...
		function resolveCurrentModel(sessionId, cb) {
			...
			// 新版（0.1.2+）优先：
			//  主路：sessions.binding(sessionId).session.projections.faceOf("modelSelection").getSnapshot().next
			//    （官方 model-selection 同口径；sessions 是普通客户端服务，取模型不依赖 remote 命名空间挂载）
			//  兜底：remote.session.modelCatalog().default（部署默认模型；remote.session 见 apply 处双路获取）
			var adopted = null;
			try {
				var ss = sessionsRef.current;
				if (ss && typeof ss.binding === "function") {
					var binding = ss.binding(sessionId);
					var proj = binding && binding.session && binding.session.projections;
					var ms = proj && typeof proj.faceOf === "function" ? proj.faceOf("modelSelection").getSnapshot() : null;
					if (ms && ms.next && ms.next.provider && ms.next.model) adopted = ms.next;
				}
			} catch (_e) { /* 忽略 */ }
			if (adopted) { done(adopted); return; }

			// 兜底：remote.session.modelCatalog().default
			...
			if (rs && typeof rs.modelCatalog === "function") {
				Promise.resolve().then(function () { return rs.modelCatalog(); }).then(function (res) {
					...
					if (def && def.provider && def.model) done(def); else legacy();
		...
		function TokenMonitorEntry(props) {
			...
			// CC-switch 历史导入（用量数据存储设计.md §6）：幂等，重复点击无副作用。
			...
			// 手动刷新带 force=1：绕过服务端 60s 缓存，否则点刷新拿到的还是缓存旧数据
			var refresh = React.useCallback(function (silent) {
				if (!silent) setRefreshing(true);
				fetch(silent ? OVERVIEW_URL : OVERVIEW_URL + "?force=1", { headers: { accept: "application/json" } })
					.then(function (r) { return r.ok ? r.json() : Promise.reject(new Error("HTTP " + r.status)); })
					.then(function (d) {
						setOverview(d);
				...
				// 手动刷新（silent=false）时同时刷当前提供方快速通道（也带 force 绕缓存）：
				// 弹层显示的 currentProvider 优先取 currentData（/overview/current 缓存），
				// 只刷全量 overview 不会更新它（余量/加油包看着"没刷新"的原因）
				if (!silent) {
					var pid = currentPidRef.current;
					if (pid) refreshCurrent(pid, true, true);
				}
			}, []);

			// 当前提供方快速刷新：只抓当前会话所用提供方（徽标优先，不等全量）。
			// 返回 { provider, pluginSettings, usdCnyRate }；provider 为单个 payload。
			// force=true：带 force=1 绕过服务端 60s 缓存（手动刷新链路用）
			var refreshCurrent = React.useCallback(function (pid, silent, force) {
				if (!pid) return;
				if (!silent) setRefreshing(true);
				fetch("/token-monitor/overview/current?provider=" + encodeURIComponent(pid) + (force ? "&force=1" : ""), { headers: { accept: "application/json" } })
					.then(function (r) { return r.ok ? r.json() : Promise.reject(new Error("HTTP " + r.status)); })
					.then(function (d) {
						if (d && d.ok) {
							setCurrentData(d);
			...
			// label       = 桌面文案（"{模型名} · 余量"）
			// labelShort  = 窄屏文案（去掉模型名，只留余量本身；fallback 链与桌面完全一致）
			// 两者同时渲染，由 scoped CSS 的媒体查询切换显隐——不需要 JS 监听窗口宽度。
			var label;
			var labelShort;
			// 文字状态色（原状态圆点已移除）：只有"告急红"一档落到文字上，
			// 正常/无数据用默认字色，未配置/未适配用灰；规则见 badgeTextState。
			var textState = "muted";
			if (fetchError && !overview) {
				...
				var quotaValue = currentProvider.ok
					? (quotaText(currentProvider, now) || locServer(currentProvider.headline) || "—")
					: providerErrText(currentProvider);
				// 模型名去供应商前缀（deepseek/deepseek-v4.1-flash → deepseek-v4.1-flash）：
				// 前缀是冗余信息（徽标本身就在显示当前供应商），去掉后徽标文字大幅缩短，
				// 配合下方"不设 max-width"让余量数值完整可见
				var modelLabel = current
					? String(current.model || "").replace(/^[^/]+\//, "")
					: currentProvider.label;
				label = modelLabel + " · " + quotaValue;
				// 移动端徽标（labelShort）：不带倒计时，只留"5h 余 99%"，避免超宽
				labelShort = currentProvider.ok
					? (quotaText(currentProvider, now, true) || locServer(currentProvider.headline) || "—")
					: quotaValue;
				textState = badgeTextState(currentProvider);
			} else if (current) {
				label = t("entry.modelQuota", { model: current.model });
				labelShort = t("entry.quotaMonitor");
			} else {
				label = t("entry.quotaMonitor");
				labelShort = label;
			}
			...
			// 触发胶囊：外层负责边框/圆角/拉伸（结构照官方 .CAgGvG_split），
			// 内层按钮负责居中内容（.CAgGvG_main）。颜色随状态（正常=默认字色 /
			// 告急=红 / 未配置=灰），hover 叠在内层按钮上（与官方 hover 同位置）。
			var triggerStyle = Object.assign({}, S.trigger, { color: S.stateColor(textState) });
			if (hovered) triggerStyle.background = "var(--dsw-alias-interactive-bg-hover)";
			...
			return h("div", { ref: rootRef, style: S.root },
				h("div", { style: S.pill },
					h("button", {
						...
						h("span", { className: "tm-entry-label tm-entry-label-full" }, label),
						h("span", { className: "tm-entry-label tm-entry-label-short" }, labelShort))),
				open
					? h("div", { className: "tm-menu", style: S.menu, role: "dialog", "aria-label": t("entry.title") },
		...
		/** 取某来源「最近更新」的时刻：与「最近同步」同源——同一条 sync_logs 行的 finished_at。
		 *  服务端不再单独下发 lastUpdated（那是同一个值的重复字段），两列都从 lastSync 读。 */
		function lastSyncAt(sources, source) {
			var row = sources && sources.lastSync ? sources.lastSync[source] : null;
		 ...
		 */
		/** 用量页视图页签：[id, 文案 key]；label 在渲染时按当前语言解析。
		 *  第一个页签是默认页：供应商统计（趋势 + 热力 + 供应商柱状图）。 */
		var USAGE_VIEWS = [
			["provider", "prov.title"],
			["rank", "rank.title"],
			["records", "rec.title"],
		 ...
		 *  deepseek-account 在此保留完整名（弹层列表/当前卡片要区分两个渠道的账户）；
		 *  统计与明细/组合列的统一名走 providerLabel 里的特例。 */
		var PROVIDER_LABELS = {
		...
		function providerLabel(id) {
			// 账号渠道与单独配置的 key 同属 DeepSeek 官方：用量统计、明细、组合列统一显示
			// DeepSeek。这里走特例而非改 PROVIDER_LABELS——后者会被服务端 provider_name
			// 覆盖，而服务端需要保留完整名给弹层的提供方列表 / 当前卡片（两个渠道可能是
			// 不同账户，余额不能混同）。
			if (id === "deepseek-account") return "DeepSeek";
		...
		var VENDOR_OF = {
			...
			deepseek: "deepseek",
			// 账号登录渠道与 API key 渠道同属 DeepSeek 官方：统计/筛选按供应商归并（离线兜底）
		...
		var SERVER_EN_RULES = [
			...
			[/^已用 /, "used "],
			[/已用 (.+?) \/ (.+)/, "used $1 / $2"],
			// —— Kimi 新会员体系（30 天周期 + 加油包）——
			[/30天编程用量/, "30d code usage"],
			[/30天用量/, "30d usage"],
			[/加油包（30天）/, "Booster (30d)"],
			[/加油包/, "Booster"],
			[/已开启/, "enabled"],
		...
		function baseTooltip() {
			return {
				...
				extraCssText: MENU_SURFACE_CSS + "border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,0.18);padding:6px 10px;line-height:18px;",
			};
		}
		...
		function buildTrendOption(data, metric) {
			...
			// 单点数据显示圆点：折线至少要 2 个点才画得出来，只有 1 个点时若也隐藏圆点，
			// 图上就完全空白（y 轴却按该点值缩放，看着像"有刻度的空图"）。多点仍是只留线条与渐变面积。
			var showSymbol = data.length < 2;
			...
			// 桶刻度 → tooltip 区间：全部窗口的周聚合桶带 range 字段（"MM-DD~MM-DD"）；
			// 当天/按天的桶没有 range，仍走 trendRangeLabel 原逻辑
			var rangeOf = {};
			for (var ri = 0; ri < data.length; ri++) {
				if (data[ri] && data[ri].range) rangeOf[data[ri].day] = data[ri].range;
			}
			...
			return {
				...
				tooltip: Object.assign(baseTooltip(), {
					...
					formatter: function (params) {
						...
						// 头部：聚合桶（周/月）用 range 显示区间；否则有 step 显示桶区间（15:00~15:30），否则原样刻度
						var headText = rangeOf[params[0].axisValue] || trendRangeLabel(params[0].axisValue, stepMin);
						var lines = [tooltipHead(headText)];
						var total = 0;
		 ...
		 *  cell：格子边长（调用方按容器可用宽度 ÷ 周数算出，宽高同值）。
		 *    ⚠️ 实测（ECharts 5.6 + SSR 验证）：只要 option 给了 calendar 的 left/top，
		 *    ECharts 会把对应方向的 cellSize 当 "auto" 处理——格子尺寸完全由**容器尺寸**
		 *    反算（宽 = (容器宽−left−right)/周数，高 = (容器高−top−bottom)/7）。
		 *    所以"格子是否正方形"真正取决于**容器高度**：调用方必须传
		 *    height = cell×7 + top + bottom；cellSize 只作表意（同值传入，防未来语义变化）。 */
		function buildCalendarOption(agg, bgColor, cell) {
			...
			return {
				...
				calendar: {
					...
					// 宽高同值；实际尺寸由容器决定（见本函数头注释），故高度必须按
					// cell×7 + top + bottom 传，否则宽高各算各的 → 格子变形
					cellSize: [cell, cell],
		...
		function CalendarHeatmap(props) {
			...
			// 容器可用宽度：格子边长由它反算（ECharts 忽略 cellSize，按容器布局——
			// 见 buildCalendarOption 头注释的实测结论）。
			var boxRef = React.useRef(null);
			var _cw = React.useState(0), boxW = _cw[0], setBoxW = _cw[1];
			React.useEffect(function () {
				var el = boxRef.current;
				if (!el) return undefined;
				setBoxW(el.clientWidth || 0);
				if (typeof ResizeObserver === "undefined") return undefined;
				var ro = new ResizeObserver(function (entries) {
					var w = entries[0] && entries[0].contentRect ? entries[0].contentRect.width : el.clientWidth;
					setBoxW(function (prev) { return Math.abs(prev - w) < 0.5 ? prev : w; });
				});
				ro.observe(el);
				return function () { ro.disconnect(); };
			}, []);

			// 周数：range 含首尾，按 7 天/列（calendar horizontal 布局）
			var weeks = React.useMemo(function () {
				var s = Date.parse(agg.rangeStart), e = Date.parse(agg.rangeEnd);
				if (!isFinite(s) || !isFinite(e) || e < s) return 53;
				return Math.max(1, Math.ceil((Math.round((e - s) / 86400000) + 1) / 7));
			}, [agg.rangeStart, agg.rangeEnd]);

			// 格子缝隙色：渲染期同步取（主题令牌优先），不再走 state+effect 实测——
			// 那条路径会在切主题时晚一帧，表现为"间隙闪一下"，详见 heatGapColor 的注释。
			var bg = heatGapColor(boxRef.current);

			// 布局：cell 由容器宽度反算，**图表高度必须 = cell×7 + top + bottom**——
			// 这是让格子成为正方形的唯一开关（ECharts 宽高两个方向各自按容器反算，
			// 高度不配合就一宽一高，必然变形）。
			//  · 常态：内层 100% 宽，格子恒为正方形；
			//  · 超宽（cell > 24）：锁上限并居中，避免格子过大；
			//  · 超窄（cell < 6）：锁下限，内层固定宽 + 横向滚动，保证可读。
			var CAL_L = 44, CAL_R = 12, CAL_T = 26, CAL_B = 4, CELL_MIN = 6, CELL_MAX = 24;
			var layout = React.useMemo(function () {
				if (!boxW) return { cell: 12, height: 12 * 7 + CAL_T + CAL_B, innerW: 0, scroll: false };
				var cell = (boxW - CAL_L - CAL_R) / weeks;
				if (cell > CELL_MAX) {
					return { cell: CELL_MAX, height: CELL_MAX * 7 + CAL_T + CAL_B, innerW: weeks * CELL_MAX + CAL_L + CAL_R, scroll: false };
				}
				if (cell < CELL_MIN) {
					return { cell: CELL_MIN, height: CELL_MIN * 7 + CAL_T + CAL_B, innerW: weeks * CELL_MIN + CAL_L + CAL_R, scroll: true };
				}
				return { cell: cell, height: Math.round(cell * 7 + CAL_T + CAL_B), innerW: 0, scroll: false };
			}, [boxW, weeks]);

			var serialized = JSON.stringify({ b: agg.byDay, l: LANG, d: isDark(), g: bg, c: layout.cell });
			var option = React.useMemo(function () {
				return buildCalendarOption(agg, bg, layout.cell);
			}, [serialized, themeRev]);
			...
			return h("div", null,
				h("div", { style: { display: "flex", alignItems: "center", gap: "2px" } },
					...
					h("span", { style: { display: "inline-flex", alignItems: "center", gap: "3px", fontSize: "10px", color: "var(--dsw-alias-label-tertiary)" } },
						...
						t("heat.high"))),
				// 内层：常态 100% 自适应；上限/下限时才固定宽度（居中或滚动）
				h("div", { ref: boxRef, style: layout.scroll ? { overflowX: "auto" } : undefined },
					h("div", { style: layout.innerW
						? { width: layout.innerW + "px", margin: layout.scroll ? undefined : "0 auto" }
						: undefined },
						h(EChart, { option: option, serialized: serialized, height: layout.height }))));
		}
		...
		/**
		 * 请求记录卡的分页数据缓存（模块级，跨"页签切走再切回"的重挂载）。
		 * 为什么需要：请求记录现在是页签面板，切走即卸载、组件内的 data 状态随之丢失，
		 * 切回重新请求 → 先渲染"加载中…"再出表，屏幕会闪一下；而供应商统计/使用排行的
		 * 数据来自父组件 state（dist/rank），卸载不丢，所以那两块不闪。
		 * 失效时机：UsageView 的 refreshKey 自增（手动刷新 / 导入 / 删除 CC 或 DSH 数据）
		 * 与筛选条件变化（由父组件重挂载整个卡片，见 key 的用法）。
		 */
		var requestRecordsCache = { key: null, page: 1, pageSize: "10", data: null, err: null };

		/** 请求记录卡：分页明细表（每页 10 条，时间倒序）。 */
		function RequestRecords(props) {
			// 初始化即取缓存：重挂载后首帧就有数据，不出现"加载中…"的闪烁；
			// 同时恢复页码/每页条数，避免切回来看到"加载中"与跳回第 1 页。
			var cached = requestRecordsCache.key === props.refreshKey && requestRecordsCache.data
				? requestRecordsCache : null;
			var _page = React.useState(cached ? cached.page : 1), page = _page[0], setPage = _page[1];
			var _size = React.useState(cached ? cached.pageSize : "10"), pageSize = _size[0], setPageSize = _size[1];
			var _data = React.useState(cached ? cached.data : null), data = _data[0], setData = _data[1];
			var _err = React.useState(cached ? cached.err : null), err = _err[0], setErr = _err[1];

			// refreshKey：父组件（UsageView）在刷新/导入/删除后自增 —— 缓存随之失效并重新拉取。
			// refreshKey 变化时清缓存：否则初始化若恰好命中旧 key 会把过期数据当首帧。
			React.useEffect(function () {
				if (requestRecordsCache.key !== props.refreshKey) {
					requestRecordsCache = { key: props.refreshKey, page: 1, pageSize: "10", data: null, err: null };
				}
			}, [props.refreshKey]);

			// 拉取后写回缓存（失败也写：切回来至少还看得见上次的错误文案，而不是空屏）
			React.useEffect(function () {
				var disposed = false;
				fetch("/token-monitor/usage/requests?page=" + page + "&size=" + pageSize, { headers: { accept: "application/json" } })
					.then(function (r) { return r.ok ? r.json() : Promise.reject(new Error("HTTP " + r.status)); })
					.then(function (j) {
						if (disposed || !j || !j.ok) return;
						setData(j.data);
						requestRecordsCache.data = j.data;
						requestRecordsCache.err = null;
					})
					.catch(function (e) {
						if (disposed) return;
						var msg = String((e && e.message) || e);
						setErr(msg);
						requestRecordsCache.err = msg;
					});
				return function () { disposed = true; };
			}, [page, pageSize, props.refreshKey]);

			// 页码/每页条数写回缓存（切走再切回时原样恢复）
			React.useEffect(function () {
				requestRecordsCache.page = page;
				requestRecordsCache.pageSize = pageSize;
			}, [page, pageSize]);
		...
		function UsageTable(props) {
			...
			React.useEffect(function () {
				...
				// 半透明菜单底必须配菜单滤镜才有实底（详见 MENU_SURFACE 注释）
				st.backdropFilter = "var(--dsw-menu-backdrop-filter, none)";
				st.webkitBackdropFilter = "var(--dsw-menu-backdrop-filter, none)";
		...
		/** 主区"用量"数据面板（用量数据存储设计.md §8 终态：时间窗 + 大数字卡 + 趋势 + 排行 + 会话明细 + 费用）。 */
		function UsageView() {
			...
			// 用量页视图页签：默认落在"供应商统计"（那里放趋势 + 热力 + 柱状图），
			// 统计卡常驻页签条之上，页签只切换下方的重内容面板。
			// 只做条件渲染、不隐藏容器——图表在隐藏容器里 init 会拿到 0×0，条件挂载天然避开；
			// 切走即卸载、EChart 的 cleanup 会 dispose，切回重新 init。
			var _vt = React.useState("provider"), usageViewTab = _vt[0], setUsageViewTab = _vt[1];
			...
			// 数据来源卡刷新信号：导入/删除/刷新成功后自增，驱动 /usage/sources 重拉。
			// 该卡原先只在挂载时拉一次（effect 依赖为空），于是导入完那两列（最近更新 /
			// 最近同步）和折叠健康告警都要刷页才更新——而它们正是"导入成没成"的直接证据。
			var _sourcesKey = React.useState(0), sourcesKey = _sourcesKey[0], setSourcesKey = _sourcesKey[1];
			...
			React.useEffect(function () {
				function onUsageRefresh() {
					...
					setSourcesKey(function (k) { return k + 1; }); // 数据来源卡同步重拉（最近更新 / 最近同步 / 折叠健康）
				}
			...
			React.useEffect(function () {
				...
				fetch("/token-monitor/usage/sources", { headers: { accept: "application/json" } })
					...
					.catch(function () {});
				return function () { disposed = true; };
				// sourcesKey 变化即重拉：导入/删除/刷新成功后自增，
				// 让「最近更新 / 最近同步」与折叠健康告警无需刷页即可反映
			}, [sourcesKey]);
			...
			var totals = React.useMemo(function () {
				var t = { requests: 0, tokens: 0, cost: 0, ttftSum: 0, ttftCount: 0, ttft: null,
			...
			/** 本地日期串 "YYYY-MM-DD" → 本地 Date（避免 new Date(str) 按 UTC 解析的偏差）。 */
			function parseDay(s) {
				var p = String(s).split("-");
				return new Date(+p[0], +p[1] - 1, +p[2]);
			}
			/** 所在周的周一（"YYYY-MM-DD"）：全部窗口按周聚合时的桶起点。 */
			function mondayOf(s) {
				var dt = parseDay(s);
			...
			/** 周桶的区间标签 "MM-DD~MM-DD"（周一 ~ 下周一，即区间结束边界）：tooltip 显示。 */
			function weekRangeLabel(mondayStr) {
				var p = function (x) { return String(x).padStart(2, "0"); };
				var fmt = function (d) { return p(d.getMonth() + 1) + "-" + p(d.getDate()); };
				var start = parseDay(mondayStr);
				return fmt(start) + "~" + fmt(new Date(start.getTime() + 7 * 86400000));
			}
			/** 日期串 + n 天（"YYYY-MM-DD"）：周桶刻度取区间结束边界（周一 + 7）。 */
			function dayPlus(s, n) {
				var dt = parseDay(s);
			...
			var trend = React.useMemo(function () {
				...
				// "全部"时间窗：不补零（跨度可能很大，没有"连续时间轴"概念），且按数据跨度
				// 自适应粒度——≤31 天按天、32~180 天按周、>180 天按月；否则几十上百个天桶
				// 挤成一团、看不出趋势（数据随时间继续增长会自动升粒度）。
				if (days === 0) {
					var keys = Object.keys(map).sort();
					if (keys.length === 0) return [];
					var spanDays = Math.round((parseDay(keys[keys.length - 1]) - parseDay(keys[0])) / 86400000);
					if (spanDays <= 31) return keys.map(function (k) { return map[k]; });
					var bucketKeyOf = spanDays > 180
						? function (dayStr) { return dayStr.slice(0, 7); } // 月：YYYY-MM
						: mondayOf; // 周：所在周的周一（YYYY-MM-DD）
					var bmap = {};
					var order = [];
					for (var bi = 0; bi < keys.length; bi++) {
						var e = map[keys[bi]];
						var bk = bucketKeyOf(e.day);
						var b = bmap[bk];
						if (!b) {
							// day = 桶刻度（x 轴显示，取区间**结束边界**，与"当天"刻度=终点同款）：
							//   周桶是下周一（周一 + 7）、月桶是 YYYY-MM（月本身就是区间，刻度不变）；
							// range = tooltip 显示的区间（"开始~结束"）：只有周桶需要
							b = bmap[bk] = { day: spanDays > 180 ? bk : dayPlus(bk, 7), cost: 0, requests: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0 };
							if (spanDays <= 180) b.range = weekRangeLabel(bk);
							order.push(bk);
						}
						b.cost += e.cost;
					...
					return order.map(function (k) { return bmap[k]; });
				}
			...
			if (loading) {
				...
				body = h("div", { style: { display: "flex", flexDirection: "column", gap: "12px" } },
					...
					// 费用口径说明：独立一行，位置在统计卡之下、趋势图之上（不放进趋势卡内部）
					...
					// —— 页签条恒在热力图下面：上面三块常驻，下面只切换重内容面板 ——
					h("div", { style: S.usageTabs, role: "tablist" },
						USAGE_VIEWS.map(function (v) {
							var active = v[0] === usageViewTab;
							return h("button", {
								key: v[0], type: "button", role: "tab", "aria-selected": active,
								style: active ? Object.assign({}, S.usageTab, S.usageTabActive) : S.usageTab,
								onMouseDown: function (e) { e.preventDefault(); },
								onClick: function () { setUsageViewTab(v[0]); },
							}, t(v[1]));
						})),
					usageViewTab === "provider" ? h("div", { style: S.usageCard },
						h(ModelBarsView, { data: dist }))
					: usageViewTab === "rank" ? h("div", { style: S.usageCard },
						h("div", { style: { display: "flex", alignItems: "center", gap: "2px" } },
						...
						h("div", { style: { display: "flex", alignItems: "center", gap: "2px" } },
							...
							h("thead", null,
								h("tr", null,
									...
									h("th", { style: Object.assign({}, S.usageTh, { width: "14%" }) }, t("src.col.lastUpdate")),
									h("th", { style: Object.assign({}, S.usageTh, { width: "7%" }) }, t("src.col.lastSync")),
									h("th", { style: Object.assign({}, S.usageTh, { width: "13%" }) }, t("src.col.action")))),
							h("tbody", null,
								h("tr", null,
									...
									h("td", { style: Object.assign({}, S.usageTd, { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }) },
										fmtLastUpd(lastSyncAt(sources, "dsh-logs"))),
									h("td", { style: Object.assign({}, S.usageTd, { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }) },
										fmtSyncRows(sources && sources.lastSync ? sources.lastSync["dsh-logs"] : null)),
									h("td", { style: Object.assign({}, S.usageTd, { display: "flex", alignItems: "center", gap: "6px" }) },
										h("button", {
											type: "button", style: S.sourceIconBtn, className: "tm-source-icon-btn", title: t("src.openDir"),
											onClick: function () { openSourceDir("dsh"); },
										}, folderIcon()),
										h("span", { style: { position: "relative", display: "inline-flex" } },
											h("button", {
												type: "button", title: t("src.import"), className: "tm-source-icon-btn",
												style: dshBusy ? Object.assign({}, S.sourceIconBtn, { opacity: 0.55, cursor: "default" }) : S.sourceIconBtn,
												disabled: !!dshBusy,
										...
										h("span", { style: { position: "relative", display: "inline-flex" } },
											h("button", {
												type: "button", title: t("src.export"), className: "tm-source-icon-btn",
												style: dshBusy ? Object.assign({}, S.sourceIconBtn, { opacity: 0.55, cursor: "default" }) : S.sourceIconBtn,
												disabled: !!dshBusy,
								...
								h("tr", null,
									...
									h("td", { style: Object.assign({}, S.usageTd, { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }) },
										fmtLastUpd(lastSyncAt(sources, "cc-switch"))),
									h("td", { style: Object.assign({}, S.usageTd, { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }) },
									...
									h("td", { style: Object.assign({}, S.usageTd, { display: "flex", alignItems: "center", gap: "6px" }) },
										...
										h("span", { style: { position: "relative", display: "inline-flex" } },
											h("button", {
												type: "button", title: t("src.import"), className: "tm-source-icon-btn",
												style: ccBusy ? Object.assign({}, S.sourceIconBtn, { opacity: 0.55, cursor: "default" }) : S.sourceIconBtn,
												disabled: !!ccBusy,
										...
										h("span", { style: { position: "relative", display: "inline-flex" } },
											h("button", {
												type: "button", title: t("src.delete"), className: "tm-source-icon-btn tm-source-danger",
												"data-cc-del": "1",
												style: ccBusy
													? Object.assign({}, S.sourceIconBtn, { color: "var(--dsw-alias-error, #ea4335)", borderColor: "rgba(234,67,53,0.35)", opacity: 0.55, cursor: "default" })
													: Object.assign({}, S.sourceIconBtn, { color: "var(--dsw-alias-error, #ea4335)", borderColor: "rgba(234,67,53,0.35)" }),
												disabled: !!ccBusy,
		...
		// 数据来源操作列图标按钮交互（inline style 不支持 :hover，幂等注入一次）：
		// 普通图标 hover 加深 + 浅底；删除（tm-source-danger）hover 红色加深 + 浅红底；禁用降透明度。
		if (typeof document !== "undefined" && !document.querySelector("style[data-plugin-css=\"tm-source-icon-btn\"]")) {
			var srcTag = document.createElement("style");
			srcTag.dataset.pluginCss = "tm-source-icon-btn";
			srcTag.textContent =
				".tm-source-icon-btn:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l1);background:var(--dsw-alias-interactive-bg-hover)}" +
				".tm-source-icon-btn.tm-source-danger:hover:not(:disabled){color:var(--dsw-alias-error,#ea4335);border-color:rgba(234,67,53,0.55);background:rgba(234,67,53,0.08)}" +
				".tm-source-icon-btn:disabled{opacity:.55;cursor:default}";
			document.head.appendChild(srcTag);
		}

		// 隐藏原生滚动条（仍可用滚轮/触摸滚动）：设置页与用量页共用，独立 style 标签
		// 保证 HMR 后仍能注入。
		//  · 设置页：展开"已适配供应商"后滚动条占位会压缩内容宽度，导致布局左右跳动；
		//  · 用量页：内容长、滚动条常驻会挤压图表与表格宽度。
		// 注意只隐藏**主滚动条**：请求记录表格自己的横向滚动条（overflowX:auto）保留，
		// 窄屏下它是"这张表还能往右看"的唯一可见线索。
		if (typeof document !== "undefined" && !document.querySelector("style[data-plugin-css=\"tm-noscroll\"]")) {
			var scrollTag = document.createElement("style");
			scrollTag.dataset.pluginCss = "tm-noscroll";
			scrollTag.textContent =
				".tm-settings-scroll,.tm-usage-scroll{scrollbar-width:none;-ms-overflow-style:none}" +
				".tm-settings-scroll::-webkit-scrollbar,.tm-usage-scroll::-webkit-scrollbar{display:none;width:0;height:0}";
		...
		/** lucide 图标（内联 svg，stroke 跟随 currentColor）：数据来源表格操作用。
		 *  inline-block + verticalAlign:middle：参与按钮行盒（sourceBtn 的 lineHeight:16px），
		 *  与同行的文字按钮高度对齐。pathDs 为 lucide 图标的 path d 数组。 */
		function lucideIcon(pathDs) {
			return h("svg", {
				width: 14, height: 14, viewBox: "0 0 24 24", fill: "none",
				stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round",
				style: { flex: "none", display: "inline-block", verticalAlign: "middle" },
			}, pathDs.map(function (d, i) { return h("path", { key: i, d: d }); }));
		}

		/** lucide folder：打开目录。 */
		function folderIcon() {
			return lucideIcon(["M20 20a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2Z"]);
		}

		/** lucide import：导入（框 + 向下箭头进入）。 */
		function fileInputIcon() {
			return lucideIcon(["M12 3v12", "m8 11 4 4 4-4", "M8 5H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-4"]);
		}

		/** lucide export：导出——直接复用 import 路径，整个图标上下翻转（scale(1,-1) 以中心为轴）。 */
		function fileOutputIcon() {
			var icon = fileInputIcon();
			icon.props.style = Object.assign({}, icon.props.style, { transform: "scale(1,-1)" });
			return icon;
		}

		/** lucide trash：删除。 */
		function trashIcon() {
			return lucideIcon(["M10 11v6", "M14 11v6", "M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6", "M3 6h18", "M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"]);
		}
		...
		function ModelPricingPanel() {
			...
			return h("div", { style: S.usageCard },
				h("div", {
					...
					// 提示文案较长（含"已有记录不重算"），放开换行；不设 nowrap 以免在窄屏挤掉标题
					h("span", { style: { fontSize: "11px", color: "var(--dsw-alias-label-tertiary)", lineHeight: "16px" } },
		...
		/** 数据来源卡"最近同步"：最近一次有新增的同步导入了多少条（无记录显示 —）。 */
		...
		function SettingsView() {
			...
			return h("div", { style: { display: "flex", flexDirection: "column", height: "100%" } },
				...
				(function () {
					...
					// 版本号未就绪（首帧 overviewCache 为空、/version 未返回）时不渲染该行：
					// 避免文案从"已是最新版本"跳变为"当前版本 vX（已是最新版本）"的闪烁；
					// 就绪后一次性显示完整文案
					...
					return h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0", borderBottom: "1px solid var(--dsw-alias-border-l2)" } },
						...
						h("div", { style: { display: "inline-flex", alignItems: "center", gap: "8px" } },
							rightText
								...
								: null,
							// 已下载待重启：按钮保留但禁用（不消失，避免布局跳变；位置固定）
							h("button", {
								type: "button",
								// 可点条件：npm/github 渠道 + 有新版本 + 不在升级中 + 非"已下载待重启"
								disabled: !canUpgrade || upgrading || restartPending,
								...
								style: Object.assign({
									...
									cursor: canUpgrade && !upgrading && !restartPending ? "pointer" : "default",
								}, canUpgrade && !upgrading && !restartPending
								...
								onMouseDown: function (e) { e.preventDefault(); },
								onClick: function () {
									if (!canUpgrade || upgrading || restartPending) return;
									setUpgrading(true);
									...
									fetch("/token-monitor/upgrade", { method: "POST" })
										...
										.then(function (d) {
											if (d && d.ok) {
												// 升级成功：先重新拉 /version 刷新状态（hasUpdate=false + installed=新版本
												// → restartPending 生效、按钮禁用），再一次性渲染成功文案。
												// 两个 setState 在同一微任务里批处理，一帧完成，无中间闪烁。
												return fetch("/token-monitor/version", { headers: { accept: "application/json" } })
													.then(function (r) { return r.ok ? r.json() : null; })
													.then(function (vd) {
														if (vd && vd.ok) setVersion(vd);
		...
		function apply(ctx) {
			...
			// slots 服务：账号渠道充值按钮探测/触发官方平台页面（shell.overlay 条目）用
			try { slotsServiceRef.current = ctx.slots || null; } catch (_e0) { slotsServiceRef.current = null; }
			...
			try {
				var probeRemote = function () {
					var found = null;
					// 0.1.7+：remote.session 是 ctx.remote 命名空间下的属性（dsh-api-remotes 用
					//   ctx.remote.$mount 挂载），取法 = ctx.remote.session 或 ctx.get("remote").session；
					// 旧版（≤0.1.5）：曾以扁平服务名 remote.session 提供，走 ctx.get("remote.session")。
					// 先新后旧两路都试，互不冲突。
					try {
						var r = null;
						try { r = ctx.remote || (ctx.get ? ctx.get("remote") : null); } catch (_e0) { }
						if (r && r.session) found = r.session;
					} catch (_e1) { }
					try { if (!found && ctx.get) found = ctx.get("remote.session") || null; } catch (_e2) { }
					if (found) remoteSessionRef.current = found;
				};
				probeRemote();
				if (typeof ctx.inject === "function") {
					// 新版 remote 命名空间（0.1.7+）若晚于本插件挂载：取到 remote 后读其 .session。
					ctx.inject(["remote"], function (scope) {
						try {
							var r = (scope && scope.remote) || (scope && scope.get ? scope.get("remote") : null);
							if (r && r.session) remoteSessionRef.current = r.session;
						} catch (_e1) { /* 忽略 */ }
					});
					// 旧版扁平服务注入（≤0.1.5）：保留原路径。
			...
			// sessions 客户端服务（普通服务，Catalog 里有 binding()）：resolveCurrentModel 主路
			// 用 binding(sessionId).session.projections.faceOf("modelSelection") 取模型——官方
			// model-selection 同口径，不依赖 remote 命名空间挂载。
			try {
				sessionsRef.current = (ctx.sessions) || (ctx.get ? ctx.get("sessions") : null) || null;
			} catch (_e0) { /* 忽略 */ }
			if (!sessionsRef.current && typeof ctx.inject === "function") {
				ctx.inject(["sessions"], function (scope) {
					try {
						sessionsRef.current = (scope && scope.sessions) || (scope && scope.get ? scope.get("sessions") : null) || null;
					} catch (_e2) { /* 忽略 */ }
				});
			}
			// 主题服务接入（dsh-client-ui-theme）：getTheme 读当前外观，theme/change 时
			// 广播窗口事件驱动图表重算色板。
			// 为什么是"get + inject 双路"：`theme` 已加进本插件的 inject 列表，cactus 下
			// apply 时服务通常已就绪；但若某个版本的服务注册得比本插件晚，只用 ctx.get
			// 会一直拿到空 → themeServiceRef 永远为 null → isDark() 退化成"量 body 背景
			// 亮度"的猜测（热力图取错色板、切主题不重算）。ctx.inject 会在服务可用后
			// 回调，把这条路径补上；老版本 DSH 没有该服务时永不回调，静默保持 null。
			try {
				var bindTheme = function (svc) {
					if (!svc || typeof svc.getTheme !== "function") return;
					if (themeServiceRef.current === svc) return; // 幂等：get 与 inject 可能都命中
					themeServiceRef.current = svc;
					ctx.on("theme/change", function () {
						try { window.dispatchEvent(new Event("token-monitor:theme-change")); } catch (_e) { /* 忽略 */ }
					});
					// 立即补一次广播：服务是在挂载之后才就绪时，已挂载的图表需要重算色板
					try { window.dispatchEvent(new Event("token-monitor:theme-change")); } catch (_e2) { /* 忽略 */ }
				};
				bindTheme(ctx.get ? ctx.get("theme") : null);
				if (!themeServiceRef.current && typeof ctx.inject === "function") {
					ctx.inject(["theme"], function (scope) {
						try { bindTheme(scope && scope.get ? scope.get("theme") : scope); } catch (_e3) { /* 忽略 */ }
					});
				}
			} catch (_ignored2) { /* 无 theme 服务：走 isDark 兜底 */ }
			...
			// 徽标文字样式 + 移动端弹层定位（同一个幂等 style 标签，作用域都是本插件自己的类）：
			// - 桌面：完整文案（含模型名，已去供应商前缀）不设 max-width，自适应撑开——
			//   去前缀后文案天然短（模型名 + 余量），余量数值完整可见；
			// - 窄屏（≤640px）：隐藏完整文案，改显不含模型名的余量本身。
			// 两个 span 都渲染、由媒体查询切换显隐，因此不需要 JS 监听窗口尺寸。
			// 垂直居中三条：
			//  · align-self:center —— label 是内层按钮（inline-flex）的 flex 项，显式居中而不靠行盒；
			//  · line-height:16px —— 钉死行盒高度，字号被移动端放大时行盒不跟着变；
			//  · text-size-adjust:100% —— 关掉移动浏览器的自动字号膨胀（放大后基线一起偏，
			//    正是"电脑端正中、手机端不居中"的典型来源）。
			//
			// 移动端弹层（.tm-menu）：实测（430×932, dpr3）徽标 rect 左 233.7 右 291.0 上 58.5 下 85.5，
			// 弹层按 absolute + right:0 + 340px 会落在 left −48.5 → 左侧出屏 48.5px；且祖先链
			// （wSkVaW_root / centerCol / frame）三个 overflow:hidden 会把 absolute 弹层一并裁掉，
			// 纵向的 maxHeight:calc(100vh − 110px) 又是按"徽标在 ~60px"估的（实际 85.5px）。
			// 因此窄屏改为 position:fixed：左右各留 16px（脱离 relative 锚点与祖先裁剪），
			// top 用实测的"徽标下边 + 6px"=92px（最小高度兜底，避免头部再变高时贴到徽标上），
			// maxHeight 按真实 top 算 = 100vh − top − 16px 底部留白。
			// z-index 保持 100：与官方弹层同级、是宿主全站最大值（侧栏 11/20、头部 1~10）。
			if (typeof document !== "undefined" && !document.querySelector("style[data-plugin-css=\"tm-entry-label\"]")) {
				var entryLabelTag = document.createElement("style");
				entryLabelTag.dataset.pluginCss = "tm-entry-label";
				entryLabelTag.textContent =
					".tm-entry-label{display:inline-block;align-self:center;overflow:hidden;text-overflow:ellipsis;" +
					"white-space:nowrap;line-height:16px;-webkit-text-size-adjust:100%;text-size-adjust:100%}" +
					// 不设 max-width：模型名已去供应商前缀，文案自适应撑开，余量数值完整可见
					".tm-entry-label-short{display:none}" +
					"@media (max-width:640px){.tm-entry-label-full{display:none}.tm-entry-label-short{display:inline-block}" +
					".tm-menu{position:fixed!important;left:16px!important;right:16px!important;" +
					"width:auto!important;max-width:none!important;top:92px!important;" +
					"max-height:calc(100vh - 108px)!important}}";
				document.head.appendChild(entryLabelTag);
			}
			...
			// 主区"用量"页签（用量数据存储设计.md §8 终态）：与"对话/轨迹"并列。
