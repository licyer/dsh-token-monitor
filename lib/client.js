...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function badgeTextState(p) {
			...
			var st = p.stats;
			if (st && (st.window || st.weekly)) {
				var weekly = st.weekly;
				var src;
				if (weekly && weekly.remainingPct <= 0) src = weekly;
				else if (st.window) src = st.window;
				else src = weekly;
				var remain = src.remainingPct;
				if (typeof remain !== "number") return "none";
				if (remain <= 0) return "err";
				var weeklyRemain = weekly && typeof weekly.remainingPct === "number" ? weekly.remainingPct : null;
				if (weeklyRemain !== null && weeklyRemain <= BADGE_WEEKLY_ERR_PCT) return "err";
				return "none";
			}
