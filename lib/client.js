...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function resolveCurrentModel(sessionId, cb) {
			...
			var legacy = function () {
				if (!legacyApi || !legacyApi.sessions || typeof legacyApi.sessions.models !== "function") {
					// 诊断：0.1.1 环境这里应能走到（connection.api.sessions.models 存在）；
					// 若为空说明 apiRef 通道没拿到句柄
					if (window && window.console) console.warn("[token-monitor] legacy models unavailable", { hasApi: !!legacyApi, hasSessions: !!(legacyApi && legacyApi.sessions), hasModels: !!(legacyApi && legacyApi.sessions && typeof legacyApi.sessions.models === "function") });
					done(null);
					return;
				}
				legacyApi.sessions.models({ sessionId: sessionId }).then(function (res) {
					var r = res && res.result;
					// 诊断：打印宿主 session.models 原始返回，区分"current=null（宿主答不出）"与调用异常
					if (window && window.console) console.warn("[token-monitor] legacy models result", r);
					done(r && r.ok && r.value && r.value.current ? r.value.current : null);
				}).catch(function (e) {
					if (window && window.console) console.warn("[token-monitor] legacy models failed", e);
					done(null);
				});
			};
