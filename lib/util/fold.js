 ...
 *  只读取 [startOffset, EOF) 区间（不再整文件读入）：日志可达数十 MB，整读会同步阻塞宿主事件循环。
 *  尾部残缺帧在此被容忍。 */
export function readSessionLog(filePath, startOffset = 0) {
  ...
  if (length <= 0) {
    ...
    try {
      ...
      let got = 0;
      while (got < length) {
        const n = fs.readSync(fd, buf, got, length - got, startOffset + got);
        if (n <= 0) break; // 文件被截断：按已读到的部分处理
        got += n;
      }
      if (got < length) buf = buf.subarray(0, got);
    } catch {
      return { header: null, events: [], nextOffset: startOffset };
    } finally {
      if (fd !== undefined) {
...
/**
 * 会话日志文件名：无版本的 `session.jsonl.zstd`，或 DSH 0.1.5 起的版本化
 * `session.v<N>.jsonl.zstd`（实测写 v3）。这里不做文件名白名单，改为按目录
 * 发现 + 版本择新，后续 DSH 再推 v4/v5 时无需改代码。
 *
 * 曾经踩坑：文件名写死 `session.jsonl.zstd` + existsSync 判断，0.1.5 改名后命中数
 * 恒为 0，且 imported=0 不触发任何日志 → 采集静默停摆，页面上"最新一条"卡住不动。
 */
const SESSION_LOG_RE = /^session(?:\.v(\d+))?\.jsonl\.zstd$/;

/** 解析日志文件名的命名版本：无版本（旧命名）记 0，`session.v3.…` 记 3；不匹配返回 null。 */
...
/**
 * 在会话目录里挑选该会话的日志文件：升级瞬间新老命名可能并存，取版本号最高者。
 * 返回 { path, version }；目录里没有合法日志时返回 null。
 */
export function resolveSessionLog(sessionDir) {
  let names;
  try {
  ...
  return best === null ? null : { path: path.join(sessionDir, best), version: bestVersion };
}
 ...
 *  只读取文件头部固定窗口（不再整文件读入）：会话日志可达数十 MB，整读会同步阻塞宿主事件循环。 */
const HEADER_WINDOW_BYTES = 262144;
function readHeader(filePath) {
  let fd;
  try {
    fd = fs.openSync(filePath, 'r');
    const buf = Buffer.allocUnsafe(HEADER_WINDOW_BYTES);
    const read = fs.readSync(fd, buf, 0, HEADER_WINDOW_BYTES, 0);
    const frames = splitZstdFrames(buf.subarray(0, read));
...
export function foldSessionFile(store, pricing, filePath, logger) {
  ...
  // 换名/换版本（session.jsonl.zstd → session.v3.jsonl.zstd）时，旧的字节偏移对新文件
  // 没有意义，必须从头重折叠。record_id = sessionId:seq 是主键（INSERT OR IGNORE），
  // 重复折叠只会命中冲突、不会重复计费。
  const renamed = Boolean(watermark && watermark.log_path && watermark.log_path !== filePath);
  const startOffset = watermark && !renamed ? (watermark.last_offset || 0) : 0;
 ...
 * 扫描全部会话日志并折叠。返回聚合结果。
 *
 * @param extraRoots 额外扫描根（一次性历史回填用，例如 DSH 升级 / 目录迁移后遗留的
 *   `sessions_backup_*`）。**主根优先**：同 sessionId 只在 `$DSH_HOME/sessions` 里
 *   不存在时才折叠额外根里的副本——否则回填会把水位改写到备份路径，后续常规扫描
 *   读的是主根真实日志、增量会错乱。常规运行不传，行为与只扫主根完全一致。
 */
export function foldAllSessions(store, pricing, dshHome, logger, extraRoots = []) {
  const mainRoot = path.join(dshHome, 'sessions');
  const result = { imported: 0, skipped: 0, filesScanned: 0, sessionDirs: 0, errors: [] };

  // 收集所有项目目录（主根 + 额外根），再分两轮折叠：主根先跑并登记会话 ID。
  const sources = [mainRoot, ...extraRoots].map((root) => ({ root, isMain: root === mainRoot }));
  const firstPass = [];
  const secondPass = [];
  for (const { root, isMain } of sources) {
    ...
    for (const projectDir of fs.readdirSync(root)) {
      ...
      (isMain ? firstPass : secondPass).push(projectPath);
    }
  }

  const seenSessionIds = new Set();
  const foldProjects = (projectPaths, onlyUnseen) => {
    for (const projectPath of projectPaths) {
      ...
      for (const sessionDir of sessionDirs) {
        const dir = path.join(projectPath, sessionDir);
        try {
          if (!fs.statSync(dir).isDirectory()) continue;
        } catch { continue; }
        result.sessionDirs += 1;
        const found = resolveSessionLog(dir);
        if (!found) continue; // 不是会话目录，或该会话还没有日志
        // 额外根的去重必须发生在折叠【之前】：先探 header 拿 sessionId，主根已有的
        // 会话直接跳过，不能先折叠再丢弃——那样仍会重折叠并把水位改写到备份路径。
        if (onlyUnseen) {
          const header = readHeader(found.path);
          if (!header || seenSessionIds.has(header.id)) continue;
          seenSessionIds.add(header.id);
        }
        try {
          const r = foldSessionFile(store, pricing, found.path, logger);
          if (!onlyUnseen && r.sessionId) seenSessionIds.add(r.sessionId);
  ...
  foldProjects(firstPass, false);
  foldProjects(secondPass, true);
