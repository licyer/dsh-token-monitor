...
/**
 * 跑一次 `dsh plugin --profile <name> …`（同步，带超时）。
...
export function runDshPlugin(profile, pluginArgs) {
  ...
  try {
    if (!viaShell) {
      result = spawnSync(file, fullArgs, { ...spawnOpts, shell: false });
      ...
      result = spawnSync(file, fullArgs, { ...spawnOpts, shell: true });
    }
  } catch (error) {
    return { exitCode: 127, timedOut: false, output: String((error && error.message) || error) };
  }
  const timedOut = result.error && result.error.code === 'ETIMEDOUT';
  return {
    exitCode: typeof result.status === 'number' ? result.status : (result.error ? 127 : 1),
    timedOut,
...
export async function upgradePlugin() {
  ...
  const run = runDshPlugin(own.profile, ['update', target]);
