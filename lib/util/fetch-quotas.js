...
async function fetchZaiRegion(ctx, config, region) {
  ...
  // 智谱假 key 也返回 HTTP 200，但 body 带业务错误码（code!==0 或 success===false）
  const rdata = res.data;
  if (rdata && typeof rdata === 'object' && (rdata.code !== undefined && Number(rdata.code) !== 0 || rdata.success === false)) {
    return { ...base, ok: false, endpoint: url, error: `凭证被拒绝（${rdata.code ?? rdata.status_code ?? '401'}）`, raw: truncateRaw(rdata) };
  }
