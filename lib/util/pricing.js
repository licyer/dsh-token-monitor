/**
 * 定价模块（统一价格入口，对应 docs/PRICING-DESIGN.md §4）。
 *
 * 一个入口拿价格，内部处理"从哪里来"：
...
function findDataDir() {
  ...
  if (process.argv[1]) {
    ...
    candidates.push(path.join(dshRoot, ...rel));
  }
  // 宿主 profile 目录兜底：脱离 DSH Host 进程运行时（CLI 脚本 / 测试），argv[1] 不是
  // DSH 启动器，按 $DSH_HOME/profiles[...]/ 找（profiles 根下即扁平安装）。
  const dshHome = process.env.DSH_HOME || path.join(os.homedir(), '.dsh');
  candidates.push(path.join(dshHome, 'profiles', ...rel));
  candidates.push(path.join(dshHome, 'profiles', 'desktop', ...rel));
  candidates.push(path.join(dshHome, 'profiles', 'web', ...rel));
