...
export function openUsageStore(logger) {
  ...
  const putWatermarkStmt = db.prepare(`
    INSERT OR REPLACE INTO fold_watermarks (session_id, log_path, last_seq, file_mtime_ms, title, updated_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  function putWatermark(w) {
    putWatermarkStmt.run(w.sessionId, w.logPath, w.lastSeq, w.fileMtimeMs, w.title ?? null, Date.now());
  }
  ...
  return {
    db, dbPath,
    recordUsage, getWatermark, putWatermark, removeWatermark, transaction,
    close: () => db.close(),
  };
}
