 ...
 * rollup upsert、水位读写助手。Schema 与 用量数据存储设计.md §4 一致：
...
/** 价格版本"当前生效"的截止哨兵：9999-12-31T23:59:59.999Z（ms）；不用 NULL 表永续（查询免空值处理）。 */
export const VALID_UNTIL_OPEN = 253402300799000;
...
CREATE TABLE IF NOT EXISTS fold_watermarks (
  ...
  -- 1 = 本轮没读到文件末尾（分批上限），下轮必须无视 mtime 继续读
...
-- （已移除 provider_mappings 表：映射关系改为只维护 lib/util/provider-mappings.js 一份，
--  不再写库；旧库遗留的表启动时 DROP 清理）
...
-- 模型定价表（见 docs/模型定价表设计.md）：行 = 一个模型的一个价格版本。
-- 只由"空表首次初始化 + 后续封口/插入"写入；不参与既有表任何逻辑。
CREATE TABLE IF NOT EXISTS model_prices (
  ...
  mode            TEXT NOT NULL DEFAULT 'fixed'
                  CHECK (mode IN ('fixed', 'time')),  -- 计价方式：'fixed'=固定价（全天同价）；'time'=错峰价（按高峰窗口乘倍率，见 peak_windows）
  currency        TEXT NOT NULL DEFAULT 'USD'
                  CHECK (currency IN ('USD', 'CNY')), -- 价格币种：'USD' / 'CNY'（DeepSeek 官方）
  input_cache_hit_price INTEGER NOT NULL DEFAULT 0,  -- 输入（缓存命中）单价；P4 = 币种 ×1e-4 / 1M token
  input_price           INTEGER NOT NULL,            -- 输入（缓存未命中）单价；同上单位
  output_price          INTEGER NOT NULL,            -- 输出单价；同上单位
  cache_create_price    INTEGER NOT NULL DEFAULT 0,  -- 缓存创建/写入单价；同上单位；多数供应商不收（恒 0），Anthropic 等按需填写
  start_time      INTEGER NOT NULL,        -- 版本生效起点（ms）；调价 = 封口旧行 + 插入新行
  end_time     INTEGER NOT NULL DEFAULT ${VALID_UNTIL_OPEN}, -- 版本生效终点（ms）；当前生效版本 = 9999-12-31 哨兵值，不用 NULL（查询免空值处理）
  source          TEXT NOT NULL DEFAULT 'custom',     -- 价格来源：custom / cc-switch / pi-ai
  created_at      INTEGER NOT NULL,        -- 录入时间（ms，审计）
  peak_multiplier REAL,                    -- 高峰期间倍率（mode='time' 必填 > 0，如 2 = 高峰时段 ×2；'fixed' 为 NULL，不适用）
  peak_windows    TEXT,                    -- 高峰窗口规则 JSON（mode='time' 必填；'fixed' 为 NULL）；窗口只含 {days,start,end}，倍率见 peak_multiplier
  UNIQUE (model, start_time),
  CHECK (mode = 'fixed' OR peak_windows IS NOT NULL),
  CHECK (mode = 'fixed' OR peak_multiplier IS NOT NULL)
);
CREATE INDEX IF NOT EXISTS idx_model_prices_lookup ON model_prices (model, start_time);
`;
...
export function openUsageStore(logger) {
  ...
  // 增量迁移：fold_watermarks 补 pending 列（"本轮没读到文件末尾、下轮必须继续"）。
  // 为什么需要它：mtime 短路（mtime 没变就跳过）会把"因为分批上限而中途停下"的文件
  // 误判成"没变化"，导致尾部永远读不到。有 pending 标记后两者可区分：
  // pending=1 → 无视 mtime 继续读；pending=0 → 才允许 mtime 短路。
  if (!wmCols.includes('pending')) {
    db.exec('ALTER TABLE fold_watermarks ADD COLUMN pending INTEGER NOT NULL DEFAULT 0');
  }
  ...
  // 清理遗留的 provider_mappings 表（映射改由 provider-mappings.js 单文件维护，不再写库）
  db.exec('DROP TABLE IF EXISTS provider_mappings');
  ...
  const putWatermarkStmt = db.prepare(`
    INSERT OR REPLACE INTO fold_watermarks (session_id, log_path, last_seq, file_mtime_ms, title, last_offset, pending, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  function putWatermark(w) {
    putWatermarkStmt.run(w.sessionId, w.logPath, w.lastSeq, w.fileMtimeMs, w.title ?? null, w.lastOffset ?? 0, w.pending ? 1 : 0, Date.now());
  }

  /**
   * 该会话已入库的最大事件时间（毫秒）；无记录返回 null。
   *
   * 这是折叠器的"全读闸"：换日志文件（日志版本升级会把整段历史重写进新文件）时，
   * 只用 `event.time > 闸` 的事件才可能入库，因此新文件里那份迁移过来的历史
   * 会被整体挡掉。刻意从明细表取而不是从水位表取——水位表只是读取游标，
   * 被清掉/丢失时明细表仍能给出真值，重复计数在校验层面就不可能发生。
   */
  function maxCreatedAt(sessionId) {
    const row = db.prepare('SELECT MAX(created_at) AS m FROM usage_requests WHERE session_id = ?').get(sessionId);
  ...
  /**
   * sync_logs 保留期清理：删除 beforeMs 之前的审计行，但**每个 source 保留两行**：
   *   1. 最新一行（MAX(id)）——数据来源卡"最近更新"列要读它；
   *   2. 最近一条 `imported > 0` 的行——"最近同步"列要回溯到"上次真实同步了几条"。
   *
   * 为什么必须是两行：折叠/CC 扫描每 5 分钟一轮，绝大多数是空转（imported = 0）。
   * 若只留最新一行，最新那行一旦是空转 0，历史上所有"有数"的记录都会被删光，
   * 于是 14 天清理后"最近同步"列只能显示 0 条——回溯逻辑（服务端 ORDER BY
   * (imported > 0) DESC）就再也拿不到真值了。两行策略让回溯永远有效。
   *
   * 返回删除行数。
   */
  function pruneSyncLogs(beforeMs) {
    const result = db.prepare(`
      DELETE FROM sync_logs
      WHERE finished_at < ?
        AND id NOT IN (
          SELECT MAX(id) FROM sync_logs GROUP BY source
          UNION
          SELECT MAX(id) FROM sync_logs WHERE imported > 0 GROUP BY source
        )
    `).run(beforeMs);
  ...
  return {
    db, dbPath,
    recordUsage, getWatermark, putWatermark, removeWatermark, maxCreatedAt, deleteBySource, transaction,
    recordSyncLog, getLastSyncWatermark,
    putRollupReplace,
    pruneOldRows, pruneSyncLogs, getLastPruneAt,
    close: () => db.close(),
  };
}
