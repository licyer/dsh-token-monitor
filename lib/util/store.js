...
export function openUsageStore(logger) {
  ...
  /**
   * sync_logs 保留期清理：删除 beforeMs 之前的审计行，但**每个 source 至少留最新一行**
   * （数据来源卡"最近同步"列要读它，清了会让该列变空）。返回删除行数。
   *
   * 为什么需要：折叠与 CC 扫描各每 5 分钟写一行，每天约 576 行、一年约 21 万行，
   * 而这张表此前只有"删除某源数据"才会被动清理。由每日 prune 顺带执行。
   */
  function pruneSyncLogs(beforeMs) {
    const result = db.prepare(`
      DELETE FROM sync_logs
      WHERE finished_at < ?
        AND id NOT IN (SELECT MAX(id) FROM sync_logs GROUP BY source)
    `).run(beforeMs);
